...
grep -rn 'tenantTransaction(' services/<service>/src/modules/*/repo.ts
grep -rn 'db.transaction(async (tx)' services/<service>/src/modules/*/consumer.ts
```
For every `db.transaction` call site, check every repo function it calls: does that function use `scopedRead` (a *second* transaction) instead of taking the caller transaction directly? Also check for the `tenantTransaction` helper from `@civitasone/db` -- it is a drop-in `db.transaction()` replacement that also opens `db.transaction()` internally, so it is exactly as dangerous as `scopedRead` for this bug, just under a different name. A scan that only greps for `scopedRead` will silently miss every `tenantTransaction`-based instance.

**Before treating a name match as a real hit, check for a local shadow.** A consumer file sometimes already defines its own private, correctly Tx-safe helper function, taking `tx` as its own first parameter and reading directly, that happens to share a name with an unrelated, flagged export from some repo file. A naive scan that just greps for the function name being called inside a transaction block will flag this as a hit even though the consumer never calls the flagged export at all -- it calls its own local one instead. Confirm the actual call resolves to the flagged repo export before fixing anything -- a bare call to a name that is ALSO declared locally in the same file with tx as its first parameter is very likely already safe.
...
- **Equivalent helper under a different name.** `scopedRead` is not the only function shaped this way. `tenantTransaction` has the identical bug shape (opens its own `db.transaction()` internally) and is used as a general-purpose transaction-opening replacement across services -- a repo function built on it needs exactly the same Tx-variant treatment. Do not assume a codebase-wide grep for `scopedRead` alone is a complete audit; check what other transaction-opening helpers exist in the shared db package first.
- **A regex-based scanner can silently mis-extract a function body.** If your scan tool locates a function's opening brace with a naive `text.find("{", after_signature)`, an inline object-literal return-type annotation containing its own `{` (e.g. `): Promise<{ rows: X[]; total: number }> {`) or a generic type argument between a call and its parens (e.g. `scopedRead<{ id: string }[]>((tx) => ...)`, `cache.getOrLoad<RiskModelRow>(...)`) will fool it — it grabs the wrong brace or misses the call entirely, and the function is silently skipped as "safe" even though it genuinely opens its own transaction. This produced real false negatives across several services in one pass (missed sites in already-"complete" admin-service and workflow-service, and an entire 5-module, 34-site batch in inspection-service that an earlier pass had never reached). Fix: track combined angle-bracket/brace nesting from the parameter list onward and take the first `{` at nesting depth zero as the real body start; allow an optional `<...>` between a call name and its opening paren when matching `scopedRead(`/`tenantTransaction(`/`db.transaction(`. After fixing a scanner bug like this, **re-run it against every "complete" service, not just the one you were auditing when you found the bug** — the same blind spot was present in every earlier pass.

A scanner-limitation rescan (the false-negative sub-pattern above) afterward found further genuine sites in already-merged services: admin-service (`api-keys` `revokeKey` opening a *raw* `db.transaction()`, not named `scopedRead`/`tenantTransaction` — and, in a second rescan round after broadening the scanner further, `uploads` `listTypes` behind a `cache.getOrLoad` plus object-literal-return-type call), workflow-service (`quorum` `castVote` and `sla` `resumeTask`, both hiding behind a return-type union), court-service (`cause-list` `getCauseList` behind a `scopedRead` call with a generic type argument), cdp-service (`dsar`/`identity`/`steward`, three modules sharing an identically-named and identically-shaped `findById`), and a large batch across inspection-service's risk/encroachment/illegal-construction/survey/telemetry modules (12 functions, 34 call sites, all cache-wrapped reads the object-literal-return-type blind spot had hidden end to end). works-service was independently swept and found genuinely clean.

**Where this bit us:** notification-service (`checkQuota`/`checkDlt`, 3 call sites across 2 review passes before all were found), building-service (`submitApplication`/`issuePermit`/`decideApplication`, reintroduced in already-merged code from an earlier PR), payroll-service (loop-inside-transaction in the per-employee loan lookup during a payroll run), finance-service (5 modules in one pass), hrms-service (16 call sites in one file, plus a second batch of 9 across 6 more modules — the most bug-dense file of the whole audit), grant-service (loop-inside-transaction in PFMS reconciliation, plus the real financial gate — `hasSubmittedUcForApplication` — sharing the same bug), billing-service (`findByTenant` making two sequential `scopedRead` calls per invocation — the multi-call-per-function variant), inspection-service (13 modules audited one by one — assignment, capa, checklist, enforcement, execution, licence, planning, findings — the findings module also required a Tx variant in a different module for a cross-module call), asset-service (12 call sites across 6 modules, including a partial-fix-left-unapplied case where a consumer already correctly used a Tx-safe pattern for one call but not a second call in the same transaction block), and crm-service (the equivalent-helper sub-pattern above was found here, plus a transaction that made three separate nested-transaction calls at once). A sibling service, meeting-service, was flagged by an initial scan with 10 candidate sites that all turned out to be the local-shadow false positive above — every one of its consumer files already had its own correctly Tx-safe local helper; a full re-sweep of the whole service found zero genuine hits, and correctly needed no fix.
...
**Never revert-and-restore uncommitted changes for a sabotage check.** The standard pattern is: revert the fix to the parent commit, prove the bug reproduces, then restore via a checkout of HEAD. That restore only works if the fix was committed before the revert -- if the fix was still sitting uncommitted in the working tree, restoring from HEAD silently overwrites it with the last committed (pre-fix) state instead of bringing it back, and every subsequent broken-fix result is really testing pre-fix code while a plain status check shows no diff (since the pre-fix state is exactly what is now on disk and matches HEAD). Cost real time to diagnose once in this audit. Commit the fix -- even as a throwaway WIP commit to amend later -- before starting the revert/restore cycle, every time.

**`git checkout <commit> -- <files>` stages what it checks out — a later unrelated commit can silently absorb a still-reverted file.** This is a different trap from the one above and bit this same audit four separate times even while correctly committing the fix before each revert. Sequence: commit the fix, `git checkout <parent> -- <files>` to revert for the "before" half of a sabotage check (this both rewrites the working tree AND re-stages those files at the parent's content), confirm the bug reproduces, then `git checkout HEAD -- <files>` to restore (this too rewrites the working tree and re-stages — now correctly, at the fix's content). If, before that restore step, some other file was `git add`-ed for an unrelated reason (a follow-on test fix, say) and then `git commit` was run without restoring the reverted files first, the commit silently includes the still-staged *reverted* (pre-fix) version of the earlier files alongside the unrelated changes — `git status`/`git diff` (unstaged) shows nothing wrong afterward, since the working tree does get restored eventually, but the *commit that already happened* has the bug back in it. Symptom: a commit whose file/line-count summary looks far larger than the described change should be (e.g. "12 files changed" for what should have been 2 test-only files). The fix is cheap and mechanical: after every `git checkout HEAD -- <files>` restore, immediately `grep` the restored file for the marker your fix added (the new `...Tx` function name) to confirm the working tree is right, then explicitly `git add` those same restored files before any subsequent commit — never assume a prior restore step already staged them correctly for a commit that comes later. If a bad commit like this already happened, it is recoverable: `git checkout <pre-revert-fix-commit> -- <files>` from the last known-good commit, re-stage, and commit again — no history rewrite needed since nothing has been pushed/merged yet.

**Doing the whole audit over SSH without ever touching the local machine is possible, including for file content containing characters a plain shell heredoc would corrupt.** Pipe the edit through a small server-side script (Python works well) that reads the file, does a plain string replacement between an old and new value, and writes it back -- keep the outer delimiter unique and unrelated to anything appearing in the content being written, since the content can otherwise prematurely terminate the script. A plain string replacement inside that script never touches the remote shell interpolation rules, so backticks, dollar signs, and quotes in the file content being written are just data, not shell syntax. Reserve fetching a file to the local machine, editing it there, and copying it back for situations where the project standing instructions do not call for keeping all work server-side.
