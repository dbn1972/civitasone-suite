...
`packages/ui-kit/` — the shared, cross-app design-system package — exports exactly two components (`Button`, `AppShell`) and is imported by exactly **one** file in the whole web app. Out of ~2,945 `.tsx` files under `apps/web/src`, **510** use a raw `<button>` element instead of any shared component. The design system that's actually maintained and used lives locally, trapped inside one app: `apps/web/src/app/_components/ds/` (`EmptyState`, `ErrorState`, `HelpTip`, `PageHeader`, `Term`, `DataTable`, `ConfirmDialog`) plus `ModuleHub`/`PageShell`. This local library's adoption is broad at the page level, not narrow: **721 of 813** (**88.7%**) `page.tsx` files under `apps/web/src/app/(app)/**` import at least one component from `_components/ds/`, `ModuleHub`, or `PageShell` — `PageHeader` alone in 683 files, `DataTable` in 171 — as measured by `node scripts/design-system-adoption.mjs` on 2026-09-15 (re-run it, rather than hand-grepping, before citing these figures again; that is how the earlier "`DataTable` in 1 file, `PageHeader` in 3" claim went stale unnoticed). Adoption is uneven across individual components, though: `Term` (2 files), `HelpTip` (12), `ConfirmDialog` (10), `ErrorState` (17), and `PageShell` itself (4) remain narrowly used even though the library as a whole is not. Separately, 114 files still use a raw `<table>` instead of `DataTable` — that count is from the original, undated pass and was not re-verified for this fix.
...
The `ds/EmptyState` and `ds/ErrorState` components exist and are correctly built where wired (per the zero-training-ux-uplift requirements), but their own adoption remains narrow — `EmptyState` in 223 of 813 `page.tsx` files, `ErrorState` in just 17 (§1) — even though page-level design-system usage overall is now broad. Most pages outside each module's one retrofitted flagship screen likely still assume happy-path data with no explicit handling for "the list is empty" vs. "the fetch failed" vs. "still loading." Don't assume a screen distinguishes these three states just because it doesn't currently show a visible bug — see §2's DataSourceBadge example, which looked fine until a real failure was fed to it.
