...
test.describe('Workforce Operations — S16', () => {
  ...
  test.describe('WFH New Request Form (/hr/workforce/wfh/new)', () => {
    test('WFH form renders with date pickers', async ({ page }) => {
  ...
  test.describe('Staffing Plan (/hr/workforce/staffing-plan)', () => {
    ...
    test('staffing plan table renders with Department and Filled columns', async ({ page }) => {
      await page.goto('/hr/workforce/staffing-plan');
      // No wrapping role="region" landmark named "Staffing plan table" exists
      // here (confirmed: neither the canonical page nor the old orphan ever
      // rendered one -- this assertion was already failing pre-PR, unrelated
      // to the redirect) -- assert the columns directly instead.
