...
export default function NewPlanError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  useEffect(() => {
      ...
      <div className="card" role="alert" aria-live="assertive" style={{ maxWidth: 560 }}>
        <div className="pad" style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          <p style={{ margin: 0, fontSize: 14, color: "var(--muted)" }}>
            We couldn&apos;t load the new plan form. You can retry or head back.
          </p>
          {error.digest ? (
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            ...
            <a className="btn ghost" href="/billing/plans" style={{ minHeight: 44, display: "inline-flex", alignItems: "center" }}>
              Back to Plans
            </a>
          </div>
        </div>
