...
export default async function PipelinePage() {
  ...
  return (
    <>
      ...
      <StatGrid>
        <StatCard icon="▣" iconBg="#e0f2fe" label="Total Engagements" value={totalDeals.toLocaleString("en-IN")} />
        <StatCard icon="◈" iconBg="#dcfce7" label="Engagement Value" value={formatMoney(totalValue)} />
        <StatCard icon="◉" iconBg="#fef3c7" label="Avg Likelihood" value={`${avgLikelihood}%`} />
        <StatCard icon="△" iconBg="#fce7f3" label="High-Value Engagements" value={highValueDeals.toLocaleString("en-IN")} />
      </StatGrid>
      <KanbanBoard
        pipeline={pipeline}
        deals={deals}
