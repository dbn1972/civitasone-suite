...
export default function NewGuesthouseBookingPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setFieldError("");
    if (!UUID_RE.test(roomId.trim())) {
      setFieldError("A valid Room ID (UUID) is required.");
      return;
    }
    if (!guestName.trim()) {
      setFieldError("Guest name is required.");
      return;
    }
    if (guestRef.trim() && !UUID_RE.test(guestRef.trim())) {
      setFieldError(
        "Guest employee ref must be a valid UUID, or leave it blank.",
      );
      return;
    }
    if (!checkIn || !checkOut) {
      setFieldError("Both check-in and check-out are required.");
      return;
    }
    ...
    if (new Date(checkOutIso) <= new Date(checkInIso)) {
      setFieldError("Check-out must be after check-in.");
      return;
    }
    setSubmitting(true);
    formError.clear();
          ...
          <div className="fields">
            ...
            >
              ...
              <input
                ...
                style={inputStyle}
              />
              <span id="roomId-help" className="sub" style={{ fontSize: 12 }}>
                Enter the room&apos;s ID. A room picker is pending a rooms
                directory API.
              </span>
            </div>
            <div
            ...
            >
              ...
              <input
                ...
                placeholder="Employee UUID, if the guest is staff"
                style={inputStyle}
              />
            </div>
            <div
