...
interface ExpenditureErrorProps {
  error: Error & { digest?: string };
  reset: () => void;
}

export default function ExpenditureError({ error, reset }: ExpenditureErrorProps) {
  return (
    <main className="flex min-h-[60vh] flex-col items-center justify-center gap-6 p-8 text-center">
      ...
      <div className="space-y-2">
        <h2 className="text-xl font-semibold text-slate-800">
          Expenditure section error
        </h2>
        <p className="max-w-md text-slate-500">
          {error.message || "An unexpected error occurred in the Expenditure section."}
        </p>
        {error.digest && (
