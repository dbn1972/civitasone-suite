            ...
            {filteredCategories.flatMap((cat) =>
              cat.tiles.map((tile) => (
                ...
                >
                  <h3 className="v">{tile.title}</h3>
                  {tile.description && <div className="l">{tile.description}</div>}
                  <div className="l" style={{ fontSize: "0.6875rem", color: "var(--mut)", marginTop: 2 }}>{cat.title}</div>
                </Link>
              )),
            )}
          </div>
          {filteredCategories.length === 0 && (
            <p style={{ textAlign: "center", color: "var(--mut)", padding: "32px 0" }}>
            ...
            return (
              <section key={cat.title} style={{ borderRadius: 10, border: "1px solid var(--line)", overflow: "hidden" }}>
                ...
                >
                  ...
                  <span style={{ fontSize: 11, color: "var(--mut)", fontWeight: 400 }}>{cat.tiles.length} items</span>
