...
export function CertificationCard({
  ...
  const STATUS_STYLE: Record<typeof status, { label: string; bg: string; color: string; border: string }> = {
    valid:         { label: "Valid",           bg: "var(--goodbg, #f0fdf4)", color: "var(--good, #15803d)", border: "var(--goodbd, #86efac)" },
    expiring_soon: { label: "Expiring Soon",   bg: "var(--warnbg, #fffbeb)", color: "var(--warn, #d97706)", border: "var(--warnbd, #fcd34d)" },
    expired:       { label: "Expired",         bg: "var(--badbg, #fef2f2)",  color: "var(--bad, #dc2626)",  border: "var(--badbd, #fca5a5)" },
  };
    ...
    >
      ...
      <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
        <div>
          ...
          <p style={{ margin: "2px 0 0", fontSize: 13, fontWeight: 600, color: "var(--ink, #1e293b)" }}>{formatDate(obtainedDate)}</p>
        </div>
        {expiryDate ? (
          <div>
