...
export default function LocationsError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  useEffect(() => {
      ...
      <div className="card" role="alert" aria-live="assertive" style={{ maxWidth: 560 }}>
        <div className="pad" style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          <h1 style={{ margin: 0, fontSize: 20 }}>
            <span aria-hidden="true">⚠️</span> Something went wrong in Locations
          </h1>
          <p style={{ margin: 0, fontSize: 14, color: "var(--muted)" }}>
            We couldn’t load this part of the Locations workspace. You can retry, or head back to the module home.
          </p>
          {error.digest ? (
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            ...
            <a className="btn ghost" href="/locations" style={{ minHeight: 44, display: "inline-flex", alignItems: "center" }}>
              Back to Locations
            </a>
          </div>
        </div>
