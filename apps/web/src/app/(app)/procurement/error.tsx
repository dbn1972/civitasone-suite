...
export default function ProcurementError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  useEffect(() => {
    console.error("Procurement module error:", error);
  }, [error]);
      ...
      <div className="card" role="alert" aria-live="assertive" style={{ maxWidth: 560 }}>
        <div className="pad" style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          <h1 style={{ margin: 0, fontSize: 20 }}>
            <span aria-hidden="true">⚠️</span> Something went wrong in Procurement
          </h1>
          <p style={{ margin: 0, fontSize: 14, color: "var(--muted)" }}>
            We couldn&apos;t load this part of the Procurement module. You can retry, or head back to the workspace.
          </p>
          {error.digest ? (
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            ...
            <a className="btn ghost" href="/procurement" style={{ minHeight: 44, display: "inline-flex", alignItems: "center" }}>
              Back to Procurement
            </a>
          </div>
        </div>
