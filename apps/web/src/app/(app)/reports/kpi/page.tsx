    ...
    <main className="min-h-screen bg-slate-50 p-6 md:p-8">
      <section className="mx-auto max-w-7xl space-y-5">
        ...
        <header className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div>
            <h1 className="text-3xl font-semibold text-slate-900">KPI Tracker</h1>
            <p className="mt-1 text-sm text-slate-600">Department-wise KPI performance against targets.</p>
          </div>
        ...
        <section className="grid grid-cols-2 gap-4 md:grid-cols-4">
          <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
            ...
            <p className="mt-1 text-2xl font-bold text-slate-900">{kpis.length}</p>
          </div>
          <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
            <p className="text-sm text-slate-500">On Track</p>
            <p className="mt-1 text-2xl font-bold text-emerald-600">{onTrack}</p>
          </div>
          <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
            ...
            <p className="mt-1 text-2xl font-bold text-red-600">{offTrack}</p>
          </div>
        </section>
            ...
            <div className="space-y-3">
              {kpis.map((kpi) => (
                <div key={kpi.id} className="flex items-center gap-3">
                  <span className="w-44 shrink-0 text-right text-xs text-slate-500 truncate" title={kpi.kpiName}>
                    {kpi.kpiName}
                  </span>
