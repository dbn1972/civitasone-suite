...
export function DomainClient({ domains: initialDomains, source }: { domains: CustomDomain[]; source: "api" | "error" }) {
  ...
  function handleAddDomain(e: React.FormEvent) {
    ...
    void fetch("/api/proxy/v1/admin/custom-domains", {
      method: "POST",
  ...
  function handleVerify(id: string) {
    void fetch(`/api/proxy/v1/admin/custom-domains/${id}/verify`, { method: "POST", credentials: "same-origin" });
    setDomains((prev) => prev.map((d) => d.id === id ? { ...d, status: "verified" as const } : d));
  }

  function handleDelete(id: string) {
    void fetch(`/api/proxy/v1/admin/custom-domains/${id}`, { method: "DELETE", credentials: "same-origin" });
