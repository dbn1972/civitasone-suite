...
export default async function InstallStatusPage() {
  ...
  return (
    <div className="page-main wrap" aria-labelledby="page-heading">
      ...
      <StatGrid>
        <StatCard icon="🚀" iconBg="#eff6ff" label="Progress" value={errored ? "—" : `${progressPct}%`} />
        <StatCard icon="✅" iconBg="#ecfdf3" label="Completed" value={errored ? "—" : completed} />
        <StatCard icon="🔄" iconBg="#fffaeb" label="In Progress" value={errored ? "—" : installSteps.filter((s) => s.status === "in_progress").length} />
        <StatCard icon="⬜" iconBg="#f1f5f9" label="Pending" value={errored ? "—" : installSteps.filter((s) => s.status === "pending").length} />
      </StatGrid>
