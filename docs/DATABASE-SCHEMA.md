...
### crm — `civitas_crm`
Schema: `crm`. Large schema (contacts/leads, accounts, deals + pipelines, activities, quotations, documents, campaigns, assignment and escalation). **Only the G12 government-programme family is detailed below** — the remaining tables are described by their migrations under `services/crm-service/migrations/`. Every table is tenant-scoped with ENABLE + FORCE RLS on the `app.tenant_id` GUC.

- `crm.programmes` — id, tenant_id, programme_code varchar(64) (stable machine key, e.g. `PMAY-U-2026`; uppercased by the route before the write so `pmay-u` and `PMAY-U` cannot both be registered), name varchar(200), description, account_id (the client department; `crm.accounts` lives in the same schema but the module does **not** join to it — it records what the caller asserted, so the module stays independently extractable), contract_id (opaque reference into contract-service, **no cross-service FK**; nullable because a programme is usually registered during pursuit, before a contract exists), product_line varchar(64) (default `government`), status varchar(16) (draft/active/suspended/closed, default draft, CHECK-constrained), start_date, end_date, sponsoring_department varchar(200), coverage_scope jsonb (default `{}`, e.g. `{ "regions": ["MH"], "districts": ["Pune","Nashik"] }` — jsonb rather than a child table because coverage is read whole, never queried across programmes, and its shape differs per tenant), created_at, updated_at, created_by, updated_by, version. The first-class programme/engagement entity that opportunities, revenue and SLA reporting hang off (Spec §25.7, Journey J6). CHECK `programmes_dates_ordered` refuses `end_date < start_date`. UNIQUE (tenant_id, programme_code) — two teams cannot register the same programme twice and split its metric series. Indexed on (tenant_id, status) and (tenant_id, account_id). Migration `0086_programmes.sql`.
- `crm.programme_metrics` — id, tenant_id, programme_id, period_start date, period_end date, metric_key varchar(64), metric_kind varchar(8) (money/count/ratio, CHECK-constrained), value_minor bigint (**monetary metrics only**, minor units, serialised as a STRING on the wire so a figure above 2^53 paise cannot be rounded by a JSON parser), currency char(3), value_numeric numeric(20,6) (**counts and ratios only** — volume, coverage, exception rate, grievance rate; read as a decimal STRING, since numeric(20,6) exceeds what a JS number represents faithfully), created_at, updated_at, created_by, updated_by, version. One row = one metric for one programme over one period. Deliberately key/value rather than column-per-metric: the metric set differs per programme type and per tenant, so a new metric needs no migration. CHECK `programme_metrics_value_matches_kind` makes exactly one value column populated for the declared kind (and `currency` non-null only for money), so a reader never guesses which column carries the number; CHECK `programme_metrics_period_ordered` keeps the period ordered. **UNIQUE (tenant_id, programme_id, period_start, metric_key) is the anti-double-count guard** — a redelivered write, or an honest re-submission of a corrected figure, UPDATEs the period's row instead of appending a second one; without it one requeued message would silently double a programme's reported revenue. Indexed on (tenant_id, programme_id, period_start DESC). Migration `0087_programme_metrics.sql`.
- `crm.deals.programme_id` — nullable uuid added to the existing deals table so an opportunity can be registered **under** a programme (Journey J6). Strictly additive: no default, no backfill, no FK. The FK is omitted on purpose — `crm.deals` is written by hot paths under FORCE RLS, so an FK would take a row lock on `crm.programmes` on every deal insert and would fail closed if a programme were archived out from under a historic deal; the `crm.programme.link_deal` command validates same-tenant existence before writing instead. Partial index `idx_deals_programme` on (tenant_id, programme_id) WHERE programme_id IS NOT NULL, since the vast majority of rows are NULL. Migration `0088_deals_programme_id.sql`.
- Emits `crm.programme.created`, `crm.programme.updated`, `crm.programme.status_changed`, `crm.programme_metric.recorded`, `crm.programme.deal_linked`.
