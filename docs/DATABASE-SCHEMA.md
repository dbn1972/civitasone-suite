...
- `recommendation.cross_sell_matrix` — id, tenant_id, trigger_product_id, recommended_product_id, segment varchar(64), channel varchar(64), priority int (default 0), created_at, updated_at, created_by, updated_by, version. Product-to-product recommendation rules, optionally narrowed by segment/channel. Indexed on (tenant_id), (tenant_id, trigger_product_id), (tenant_id, priority DESC).
