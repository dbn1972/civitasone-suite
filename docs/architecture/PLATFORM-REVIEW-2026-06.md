...
| R7 | **Money downcast to JS `number` at queue/event boundaries** — precision loss > ~₹9,007 cr | D3 | 🟧 In progress | shared money codec landed (`@civitasone/schemas/money`: `parseMinor` tolerant decode + `minorString` exact encode + zod fields; 7 tests); finance queue-boundary emissions converted to exact strings (sanction.approved, payment.made via both paths, gem invoice match, GRN→bill draft no longer `Number(bigint)`); finance 126 green. Remaining: same boundary sweep in procurement(grn)/asset/tender/payroll consumers + route validators accepting decimal strings + presentation-layer display |
