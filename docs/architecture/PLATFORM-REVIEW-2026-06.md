...
| R17 | **Vendor blacklist tenant-scoped only** — CVC debarment not government-wide | D8 | ✅ FIXED | added `scope` ('tenant'|'central') + `pan` to `vendor_blacklist` (migration 0011); `isCentrallyDebarredTx(pan)` matches an active central debarment by PAN across ALL tenants; PO-create + GeM-award gates now reject a centrally-debarred firm in any tenant; `POST /v1/procurement/central-debarment` (super_admin/CVC) records by PAN, with a tenant-agnostic listing; 5 tests + procurement 75 green |
