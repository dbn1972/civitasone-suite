...
4. ~~**Backup/restore drill** to prove RPO ≤15 min / RTO ≤4 h (Charter §28.3, §38.6 "failure-testing evidence").~~ **DONE** — the `Drill_Scheduler` (`.github/workflows/dr-drill.yml`) runs `scripts/ops/backup-databases.sh` + `scripts/ops/restore-drill.sh --all-tier01` weekly (Sunday 03:00 UTC, plus on-demand via `workflow_dispatch`), verifying every Tier-0/Tier-1 backup restores with a passing table-count + sample-row check. `Drill_Report` history is retrievable from GitHub Actions artifacts (400-day retention) and the durable `civitasone` bucket's `dr-drills/` prefix (`scripts/ops/publish-drill-report.mjs`) — see [`docs/runbooks/README.md`](../runbooks/README.md#dr-restore-drill-reports-drill_report-retrieval) for the full retrieval procedure. Failures page the platform's alert channel via `scripts/ops/notify-alert-channel.mjs`.
