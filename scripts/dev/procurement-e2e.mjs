 ...
 * Usage: node scripts/dev/procurement-e2e.mjs [--base http://127.0.0.1:3000]
 */
...
async function login(page) {
  await page.goto(`${BASE}/auth/dev`, { waitUntil: 'domcontentloaded', timeout: 30000 });
  await page.fill('#username', 'superadmin');
  await page.fill('#password', 'Civitas@123');
  await Promise.all([
...
async function evaluateScreen(page, screen) {
  ...
  try {
    ...
    result.hasEmptyState = /no (records|data|items|results)|empty/i.test(bodyText);
    result.hasKpi = await page.locator('.stat, [class*="StatCard"]').count() > 0;

    if (result.hasTable) {
      const rows = await page.locator('tbody tr, .tbl tbody tr, table tbody tr').count();
    ...
    // Pass criteria
    const contentOk = result.hasTable || result.hasEmptyState || result.hasKpi || screen.type === 'hub' || screen.type === 'detail';
    const noErrors = !result.apiUnavailable && !result.errorBadge;
    if (result.httpOk && result.hasHeading && noErrors && contentOk) {
      result.status = 'PASS';
    } else {
...
async function main() {
  ...
  for (const screen of SCREENS) {
    ...
    if (r.networkFailures.length) console.log(`    → API: ${r.networkFailures[0]}`);
  }
