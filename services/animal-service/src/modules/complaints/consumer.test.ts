/**
 * Regression test: GET /v1/animal/complaints/:id serves through cache.getOrLoad
 * (60s TTL); no consumer invalidated that key on assign/dispatch/close, so a
 * citizen/officer could see a stale complaint status for up to 60s after a
 * real status change. Per @civitasone/cache's own header comment (CLAUDE.md §6):
...
vi.mock("./repo.js", () => ({
  insertComplaint: vi.fn(async () => undefined),
...
vi.mock("./domain.js", () => ({
  generateComplaintNumber: vi.fn(() => "ANC-1"),
  routeComplaint: vi.fn(() => "field-team-1"),
}));

import { registerComplaintConsumers } from "./consumer.js";
import { COMMANDS } from "../../topics.js";

const TENANT = "10000000-aaaa-4000-8000-000000000001";
const ACTOR = "20000000-bbbb-4000-8000-000000000001";
const COMPLAINT_ID = "30000000-cccc-4000-8000-000000000001";
...
describe.each([
  [COMMANDS.assignComplaint, { id: COMPLAINT_ID, assignedTo: ACTOR, assignedTeam: "field-team-1" }],
  [COMMANDS.dispatchTeam, { id: COMPLAINT_ID }],
  [COMMANDS.closeComplaint, { id: COMPLAINT_ID, resolution: "Animal relocated" }],
])("%s command", (command, payload) => {
  it("invalidates the complaint's read-through cache entry when the update matches a row", async () => {
    const q = await buildQueue();
    await q.publish(command, makeMsg(command, payload));
    await settle();
    expect(invalidateMock).toHaveBeenCalledOnce();
    expect(makeKeyMock).toHaveBeenCalledWith(TENANT, "complaint", COMPLAINT_ID);
    await q.stop();
  });
