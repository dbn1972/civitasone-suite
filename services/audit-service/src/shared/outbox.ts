...
export async function enqueue(tx: any, e: { topic: string; eventType: string; tenantId: string; actorId: string; correlationId: string; payload: Record<string, unknown> }): Promise<void> {
...
export async function relayOnce(db: any, queue: Queue, batch = 100): Promise<number> {
  ...
  for (const row of rows) {
    await queue.publish(row.topic, { type: row.eventType, tenantId: row.tenantId, actorId: row.actorId, correlationId: row.correlationId, schemaVersion: "1.0", payload: row.payload });
...
export function startRelay(db: any, queue: Queue, intervalMs = 500): NodeJS.Timeout {
  return setInterval(() => { relayOnce(db, queue).catch((err) => console.error({ err }, "outbox relay failed")); }, intervalMs);
}
