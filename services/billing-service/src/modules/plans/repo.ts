...
export async function list(): Promise<PlanView[]> {
  const rows = await scopedRead((tx) => tx.select().from(billingPlans).where(eq(billingPlans.active, true)));
  return rows.map(toView);
}

export async function findById(id: string): Promise<PlanView | null> {
  const rows = await scopedRead((tx) => tx.select().from(billingPlans).where(eq(billingPlans.id, id)).limit(1));
