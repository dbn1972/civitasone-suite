...
export async function listTickets(tenantId: string, limit: number): Promise<{ data: HelpdeskTicketSummary[]; pagination: { hasMore: boolean; pageSize: number; cursor?: string } }> {
...
export async function getSlaRules(tenantId: string): Promise<Array<{ queue: string; targetDisplay: string; breachedCount: number }>> {
  ...
  const open = counts.find((c) => c.status === "open")?.count ?? 0;
  return [
    { queue: "General", targetDisplay: "24h", breachedCount: Math.max(0, open - 5) },
    { queue: "Priority", targetDisplay: "4h", breachedCount: Math.max(0, Math.floor(open / 3)) },
  ];
}
