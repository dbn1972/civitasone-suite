...
export async function findRtiByIdTx(tx: Writer, id: string): Promise<RtiRow | null> {
  const rows = await (tx as typeof db).select().from(citizenRtiRequests).where(eq(citizenRtiRequests.id, id)).limit(1);
  return rows[0] ?? null;
}
...
export async function updateRti(tx: Writer, id: string, patch: Partial<RtiInsert>): Promise<void> {
  await tx.update(citizenRtiRequests).set({ ...patch, updatedAt: new Date() }).where(eq(citizenRtiRequests.id, id));
}
