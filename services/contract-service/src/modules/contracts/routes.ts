...
export async function contractRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.patch("/v1/contract/contracts/:id/milestones/:milestoneId/late", async (req, reply) => {
    ...
    // Penalty = delay_weeks × penaltyRate% of milestone amount, capped at maxPenalty%
    ...
    // Mark milestone late/completed with penalty metadata
    ...
    return reply.send({
      data: {
        ...
        penaltyMinor,
        milestoneAmountMinor: milestoneAmt,
        netPayableMinor,
