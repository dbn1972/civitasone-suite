...
describe("SoD — self-approval / self-termination rejected with 403", () => {
  ...
  it("maker approving own contract -> 403 SOD_VIOLATION", async () => {
    await expect(runWithTenant(TENANT, () => commands.approveContract(ctx(MAKER), id, {}))).rejects.toMatchObject({
      status: 403, code: "SOD_VIOLATION",
    });
  });

  it("maker terminating own contract -> 403 SOD_VIOLATION", async () => {
    await expect(runWithTenant(TENANT, () => commands.terminateContract(ctx(MAKER), id, { reason: "self terminate attempt" })))
      .rejects.toBeInstanceOf(HttpError);
  });

  it("a distinct checker may approve (no throw)", async () => {
    const res = await runWithTenant(TENANT, () => commands.approveContract(ctx(CHECKER), id, {}));
    expect(res.status).toBe("accepted");
  });

  it("unknown contract -> 404", async () => {
    await expect(runWithTenant(TENANT, () => commands.approveContract(ctx(CHECKER), randomUUID(), {})))
