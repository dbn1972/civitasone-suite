...
-- Mirrors visitor-service 0013_outbox_inbox_rls.sql / works-service
-- 0011_outbox_rls_if_owner.sql. Safe no-op if not owner.
--
-- Deploy note: court-worker MUST run startRelay(scannerDb, …) and
-- startOutboxPurge(scannerDb, …) (see src/worker.ts). The outbox relay/purge
-- are cross-tenant scans and court_scanner is BYPASSRLS (migration
-- 0016_court_scanner_role.sql) with grants on _outbox/_inbox.
--
-- Lesson learned from visitor-service (0013): court-service's _inbox.processed
-- (created in 0001_court_core.sql) is message_id-only — no tenant_id column —
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    RETURN;
  END IF;
  -- court-service's inbox is message_id-only (0001_court_core.sql) — no
