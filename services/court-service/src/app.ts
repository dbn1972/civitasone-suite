...
export async function buildApp(): Promise<FastifyInstance> {
  ...
  // Court-service hardening: source the RLS tenant from the AUTHENTICATED
  // token (req.ctx, populated by authPlugin's earlier onRequest hook), not the
  // client-supplied x-tenant-id header. Ensures the app.tenant_id GUC is set
  // for token-based requests so RLS enforces isolation on reads AND writes.
