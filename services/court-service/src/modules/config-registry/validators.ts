...
/**
 * Set (create or version-guarded update) a config entry (§47). `value` is
 * arbitrary JSON but MUST be present (a config entry always carries a value).
...
/** Deactivate (soft-retire) a config entry (§47). `expectedVersion` optimistic lock. */
