/**
 * court-service — transactional outbox / inbox + optimistic-locking helper.
 *
 ...
 * service's `court`-schema entities, all of which carry the standard
 ...
 * the `court` schema except the append-only audit tables
 * (`case_state_transitions`, `order_versions`, `hearing_history`) satisfies this
 * shape.
 */
 ...
 *     set: { status: "admitted", updatedBy: actorId, updatedAt: new Date() },
 *     entity: "case",
 *   });
 *   await enqueue(tx, caseStatusChangedEvent);
 * });
 */
export async function versionedUpdate<TTable extends VersionedTable>(
  ...
  args: {
    ...
    /** Human label used in the conflict error message (e.g. "case"). */
    entity?: string;
  },
): Promise<void> {
