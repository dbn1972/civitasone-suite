/**
 * Static regression: court-service migration 0015 must enable RLS on
 * `_outbox.messages` (and `_inbox.processed` when tenant_id exists),
 * guarded by table-owner checks. Mirrors visitor-service 0013 / works-service
 * 0011.
 */
...
describe("court outbox/inbox RLS migration 0015", () => {
  const sql = readFileSync(join(migDir, "0015_outbox_inbox_rls.sql"), "utf8");
  ...
  it("skips inbox RLS when tenant_id column is absent (court's _inbox.processed has none)", () => {
    expect(sql).toContain("no tenant_id column");
    expect(sql).toMatch(/information_schema\.columns/);
  });
});

describe("court scanner role migration 0016", () => {
  const sql = readFileSync(join(migDir, "0016_court_scanner_role.sql"), "utf8");

  it("creates a BYPASSRLS court_scanner role, idempotently", () => {
    expect(sql).toContain("court_scanner");
    expect(sql).toContain("BYPASSRLS");
    expect(sql).toMatch(/IF NOT EXISTS \(SELECT 1 FROM pg_roles WHERE rolname = 'court_scanner'\)/);
  });

  it("does not ship a password literal (SEC-P1-09)", () => {
    expect(sql).not.toMatch(/PASSWORD\s+'[^%'][^']*'/);
    expect(sql).toContain("civitas.court_scanner_password");
  });

  it("grants the outbox/inbox privileges startRelay + startOutboxPurge need", () => {
    expect(sql).toMatch(/GRANT[^;]*_outbox\.messages[^;]*court_scanner/s);
    expect(sql).toMatch(/GRANT[^;]*_inbox\.processed[^;]*court_scanner/s);
  });
});
