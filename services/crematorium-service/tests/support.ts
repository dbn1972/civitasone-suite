/**
 * Shared test support for crematorium-service's DB-backed integration suite.
...
export const ADMIN_ROLES = ["crematorium_admin", "super_admin"];
// A caller's ACTUAL token roles for "plain citizen" scenarios -- deliberately
// just crematorium_user, NOT the app's broader route-level USER_ROLES allow-list
// (["crematorium_user", "crematorium_admin", "super_admin"], see
// bookings/routes.ts). Handing a test citizen a token that also carries
// crematorium_admin would make every ownership/RBAC check that does
// hasAnyRole(ctx, ADMIN_ROLES) silently pass, defeating the very thing these
// tests are proving -- caught while writing this suite (an early draft used
// the app's USER_ROLES constant here and every "plain citizen" negative-path
// test false-passed).
export const CITIZEN_ROLES = ["crematorium_user"];

export function hdr(
  sub = ACTOR_A,
  tid = TENANT_A,
  roles: string[] = ADMIN_ROLES,
