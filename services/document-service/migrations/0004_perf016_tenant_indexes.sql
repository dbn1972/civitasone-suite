-- 0004_perf016_tenant_indexes.sql
-- PERF-016 (remainder of PERF-002): continues the fleet-wide tenant_id
-- leading-index backlog. This is the document-service tranche-6 file: 2
...
-- 1590 tenant_id-bearing tables fleet-wide) -- document: 2 missing,
...
--   document.file_versions (status column: no)
--   document.notings (status column: no)
--
...
-- information_schema, connected as document_svc (not civitas_admin, matching
-- tranches 1-5's methodology).
--
-- Index naming: idx_<table>_tenant[_status]. Where the table's own bare
-- name already carries the document_ prefix, it is reused verbatim (no double
-- prefixing); where it doesn't, document_ is prepended -- same rule tranches
...
-- collisions across schemas in civitas_document: none found for any table in
-- this file.
--
...
--
-- Rollback:
--   DROP INDEX CONCURRENTLY IF EXISTS document.idx_document_file_versions_tenant;
--   DROP INDEX CONCURRENTLY IF EXISTS document.idx_document_notings_tenant;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_document_file_versions_tenant
  ON document.file_versions (tenant_id);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_document_notings_tenant
  ON document.notings (tenant_id);
