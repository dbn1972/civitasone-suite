  ...
  for (const t of [TENANT_A, TENANT_B]) {
    await runWithTenant(t, () =>
      db.transaction(async (tx) => {
        await tx.delete(estabDfaTemplate).where(eq(estabDfaTemplate.tenantId, t));
...
describe("R8 DFA template library", () => {
  it("inserts and lists active templates by tenant", async () => {
    ...
    const list = await runWithTenant(TENANT_A, () => listDfaTemplates(TENANT_A));
  ...
  it("finds a template by code", async () => {
    ...
    const tpl = await runWithTenant(TENANT_A, () => findDfaTemplateByCode(TENANT_A, "DO_LETTER"));
  ...
  it("is tenant-scoped", async () => {
    ...
    const fromB = await runWithTenant(TENANT_B, () => listDfaTemplates(TENANT_B));
...
describe("R8 VIP/Parliament fields on files", () => {
  it("persists vip_reference, parliament_qno, is_vip", async () => {
    ...
    const f = await runWithTenant(TENANT_A, () => findFileById(id, TENANT_A));
