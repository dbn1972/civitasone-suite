 ...
 * carried a `bills` field. Seeds one vendor with two real bills directly via
 * the payments.finance_bills table, and TDS amounts via the REAL TDS ledger
 * (gl.finance_vendor_tds) -- not hand-seeded into the bill's own deductions
 * jsonb, which no real code path ever populates (see masters/routes.ts's
 * toVendorBillHistory for why). One bill deliberately carries a misleading
 * non-empty `deductions` value to prove the rollup genuinely ignores it
...
const OTHER_TENANT = "aaaaaaaa-2222-4000-8000-0000000000fb";
...
const TDS_ROW = "55555555-dddd-4000-8000-0000000000f1";

function token(tenant: string, roles: string[]) {
  return signToken({ sub: ACTOR, tid: tenant, roles, sid: "sess-vendor-bills" }, SECRET);
}
const financeAdmin = (tenant = TENANT) => ({ authorization: `Bearer ${token(tenant, ["finance_admin"])}` });

async function cleanup() {
  await scoped(TENANT, (tx) => tx.delete(financeVendorTds).where(eq(financeVendorTds.billId, BILL_WITH_TDS)));
...
beforeAll(async () => {
  ...
  await scoped(TENANT, (tx) => tx.insert(financeBills).values({
    ...
    // Deliberately misleading: no real code path ever writes to this field
    // (see toVendorBillHistory's comment), so a correct fix must ignore it
    // entirely rather than coincidentally agreeing with the real TDS row
    // below. If the rollup ever regresses to reading this again, this test
    // catches it by seeing 99999999 instead of the real 10000.
    deductions: [{ type: "tds", amountMinor: 99999999, description: "decoy -- must not be read" }],
  ...
  // The real TDS ledger: gl.finance_vendor_tds, keyed by bill_id.
  await scoped(TENANT, (tx) => tx.insert(financeVendorTds).values({
    id: TDS_ROW, tenantId: TENANT, vendorId: VENDOR, billId: BILL_WITH_TDS,
    grossAmountMinor: 110000n, tdsAmountMinor: 10000n, netPaymentMinor: 100000n,
    deductionDate: "2026-08-01", quarter: "Q2", fy: "2026-27",
  }));
});
afterAll(async () => { await cleanup(); await sqlClient.end(); });

describe("GET /v1/finance/vendors/:id — bill-history rollup", () => {
  it("returns a bills array sourcing TDS from gl.finance_vendor_tds, not the bill's own deductions", async () => {
    ...
    try {
      ...
      // The real ledger says 10000 -- NOT the decoy 99999999 sitting in this
      // bill's own (never-written-to-in-production) deductions field.
      ...
      // No finance_vendor_tds row at all for this bill -- defaults to 0.
  ...
  it("a different tenant cannot see this vendor (RLS + tenant-scoped lookup returns 404, not someone else's data)", async () => {
    const app = await buildApp();
    try {
      const res = await app.inject({
        method: "GET", url: `/v1/finance/vendors/${VENDOR}`, headers: financeAdmin(OTHER_TENANT),
      });
