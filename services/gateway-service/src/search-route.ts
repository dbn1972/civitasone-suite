...
const ROLE_MODULE_MAP: Record<string, string[]> = {
  ...
  // Municipal Sec5 (17 services) - role names verified against each
  // service's actual requireRole(ctx, ...) guards on main, not copied
  // unverified from an earlier draft. refund uses "_approver", not
  // "_officer" (see municipal-catalog.ts for why).
  ...
  refund_user: ["refund"], refund_admin: ["refund"], refund_approver: ["refund"],
