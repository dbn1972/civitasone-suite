...
async function wipe(tenant = TENANT) {
  ...
  await db.delete(grantUcValidations).where(eq(grantUcValidations.tenantId, tenant));
  await db.delete(grantComplianceReports).where(eq(grantComplianceReports.tenantId, tenant));
...
describe("SoD — separation of duties on approval", () => {
  ...
  it("consumer re-asserts SoD: self-approve emits sod_violation, no status flip", async () => {
    ...
    await settle();
    const rows = await db.select().from(grantApplications).where(eq(grantApplications.id, APP));
...
describe("Scheme budget — atomic reservation", () => {
  it("reserve within budget succeeds and increments disbursed_minor", async () => {
    ...
    const ok = await db.transaction((tx) => reserveSchemeBudget(tx, SCHEME, TENANT, 600n));
    expect(ok).toBe(true);
    const s = await db.select().from(grantSchemes).where(eq(grantSchemes.id, SCHEME));
    expect(s[0]!.disbursedMinor).toBe(600n);
  });

  it("reserve over budget is rejected, disbursed_minor unchanged", async () => {
    await seedScheme(1_000n);
    await db.transaction((tx) => reserveSchemeBudget(tx, SCHEME, TENANT, 600n));
    const ok = await db.transaction((tx) => reserveSchemeBudget(tx, SCHEME, TENANT, 600n)); // 1200 > 1000
  ...
  it("cross-tenant reserve cannot touch another tenant's scheme", async () => {
    ...
    const ok = await db.transaction((tx) => reserveSchemeBudget(tx, SCHEME, TENANT_B, 100n));
    expect(ok).toBe(false);
    const s = await db.select().from(grantSchemes).where(eq(grantSchemes.id, SCHEME));
  ...
  it("concurrent reservations never overspend the envelope", async () => {
    ...
    const results = await Promise.all(
      Array.from({ length: 5 }, () => db.transaction((tx) => reserveSchemeBudget(tx, SCHEME, TENANT, 300n))),
    );
...
describe("Disbursement — budget gate + double-disburse guard", () => {
  ...
  it("replayed disburse command (same messageId) is idempotent — single row", async () => {
    ...
    await settle();
    const disbs = await db.select().from(grantDisbursements).where(eq(grantDisbursements.installmentId, INST));
...
describe("UC-gate — tranche N blocks until tranche N-1 UC is VALIDATED", () => {
  it("installment #2 blocked when prior UC only submitted (not validated)", async () => {
    ...
    await db.insert(grantUcStatements).values({
  ...
  it("hasSubmittedUcForApplication only true once prior tranche UC is validated", async () => {
    ...
    await db.update(grantUcStatements).set({ validationStatus: "validated" }).where(eq(grantUcStatements.id, UC));
...
describe("Disbursement settlement + completed-only sum + PFMS reconcile", () => {
  it("sumDisbursedForApplication counts only completed disbursements", async () => {
    ...
    await db.update(grantDisbursements).set({ status: "completed" }).where(eq(grantDisbursements.pfmsTxnId, "PFMS-A"));
  ...
  it("finance.payment.made completes the disbursement and reconciles PFMS", async () => {
    ...
    await settle();
    const d = await db.select().from(grantDisbursements).where(eq(grantDisbursements.id, DISB));
    expect(d[0]!.status).toBe("completed");
    expect(d[0]!.disbursedAt).not.toBeNull();
    const pfms = await db.select().from(grantPfmsRecords).where(eq(grantPfmsRecords.disbursementId, DISB));
...
describe("Read model — disbursed/pending amounts + grant detail (regression)", () => {
  ...
  it("overdue detection: stale (>90d-old) report does NOT mark an app compliant", async () => {
    ...
    await db.update(grantApplications).set({ approvedAt: oldApproved }).where(eq(grantApplications.id, APP));
    // a compliance report exists, but it is 200 days old (outside the 90d window)
    await db.insert(grantComplianceReports).values({
