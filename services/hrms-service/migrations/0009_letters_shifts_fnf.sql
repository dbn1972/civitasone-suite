...
--
-- FIXED 2026-07-27: the `variables` values were written as ''["a","b"]'' — doubled
-- single quotes at the outer level, as if the literal were nested inside another
-- quoted string. Postgres parses that as an empty string followed by `[`, so this
-- was a syntax error and the migration aborted here on every run: the letter
-- templates and the shift seed rows below were never inserted anywhere. Corrected
