...
export async function findAccountByEmployeeTx(tx: Writer, tenantId: string, employeeId: string): Promise<NpsAccountRow | null> {
  const rows = await tx.select().from(hrmsNpsAccounts)
