...
export async function findResponse(tenantId: string, id: string): Promise<InterviewResponseRow | null> {
  return scopedRead((tx) => findResponseTx(tx, tenantId, id));
}

/** Tx-scoped variant of findResponse -- see .claude/skills/16-production-readiness-audit.md section 1. */
export async function findResponseTx(tx: Writer, tenantId: string, id: string): Promise<InterviewResponseRow | null> {
  const rows = await (tx as typeof db).select().from(hrmsInterviewResponses)
