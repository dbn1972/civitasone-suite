...
export async function listScores(tenantId: string, interviewId: string): Promise<InterviewScoreRow[]> {
  return scopedRead((tx) => listScoresTx(tx, tenantId, interviewId));
}

/** Tx-scoped variant of listScores -- see .claude/skills/16-production-readiness-audit.md section 1. */
export async function listScoresTx(tx: Writer, tenantId: string, interviewId: string): Promise<InterviewScoreRow[]> {
  return (tx as typeof db).select().from(hrmsInterviewScores)
