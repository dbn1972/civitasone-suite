...
describe("offer joining-extension + analytics routes", () => {
  it("requests a joining-date extension on an accepted offer (200)", async () => {
    ...
    const r = await app.inject({ method: "POST", url: `/v1/hrms/offers/${OFF}/joining-extension`, headers: auth, payload: { requestedJoiningDate: "2026-10-15", reason: "relocation logistics" } });
  ...
  it("refuses an extension that is not later than the current date (422)", async () => {
    H.findOffer.mockResolvedValue(offer());
    const app = await buildApp();
    const r = await app.inject({ method: "POST", url: `/v1/hrms/offers/${OFF}/joining-extension`, headers: auth, payload: { requestedJoiningDate: "2026-08-01", reason: "earlier please" } });
  ...
  it("records the requester and preserves it (requestedBy set on request)", async () => {
    H.findOffer.mockResolvedValue(offer());
    const app = await buildApp();
    await app.inject({ method: "POST", url: `/v1/hrms/offers/${OFF}/joining-extension`, headers: auth, payload: { requestedJoiningDate: "2026-10-15", reason: "relocation logistics" } });
  ...
  it("approve is senior-only and applies the new joining date (200)", async () => {
    ...
    const app = await buildApp();
    const denied = await app.inject({ method: "POST", url: `/v1/hrms/offers/${OFF}/joining-extension/approve`, headers: officer, payload: {} });
    expect(denied.statusCode).toBe(403);
    const ok = await app.inject({ method: "POST", url: `/v1/hrms/offers/${OFF}/joining-extension/approve`, headers: auth, payload: {} });
  ...
  it("blocks the requester from approving their own extension (403 SoD)", async () => {
    ...
    const app = await buildApp();
    const r = await app.inject({ method: "POST", url: `/v1/hrms/offers/${OFF}/joining-extension/approve`, headers: auth, payload: {} });
  ...
  it("restricts small-cohort analytics to senior roles (hr_officer 403, hr_admin 200)", async () => {
    ...
    const app = await buildApp();
    const denied = await app.inject({ method: "GET", url: `/v1/hrms/recruitment/offer-analytics?jobOpeningId=${JOB}`, headers: officer });
    expect(denied.statusCode).toBe(403);
    expect(denied.json().code).toBe("SMALL_COHORT");
    const ok = await app.inject({ method: "GET", url: `/v1/hrms/recruitment/offer-analytics?jobOpeningId=${JOB}`, headers: auth });
    expect(ok.statusCode).toBe(200);
    await app.close();
  });
  ...
  it("returns offer analytics for a real cohort (hr_officer allowed)", async () => {
    ...
    const app = await buildApp();
    const r = await app.inject({ method: "GET", url: `/v1/hrms/recruitment/offer-analytics?jobOpeningId=${JOB}`, headers: officer });
