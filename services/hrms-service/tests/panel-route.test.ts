...
vi.mock("../src/shared/db.js", async (io) => ({
  ...
  db: { transaction: async (cb: (tx: unknown) => Promise<unknown>) => cb({}), insert: () => ({ values: async () => undefined }) },
}));
...
describe("interview panel governance routes", () => {
  ...
  it("refuses to change the panel after an outcome is recorded (409)", async () => {
    ...
    const app = await buildApp();
    const r = await app.inject({ method: "POST", url: `/v1/hrms/interviews/${IV}/panel`, headers: auth, payload: { members: [{ memberId: CHAIR, memberName: "C", panelRole: "chair" }] } });
  ...
  it("requires a rejection reason for a rejection (422)", async () => {
    ...
    const app = await buildApp();
    const r = await app.inject({ method: "POST", url: `/v1/hrms/interviews/${IV}/outcome`, headers: auth, payload: { status: "rejected" } });
    expect(r.statusCode).toBe(422);
  ...
  it("recuses a panelist (200)", async () => {
    const app = await buildApp();
    const r = await app.inject({ method: "POST", url: `/v1/hrms/interviews/${IV}/panelists/${COIP}/recuse`, headers: auth, payload: {} });
    expect(r.statusCode).toBe(200);
