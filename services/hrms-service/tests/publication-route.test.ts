...
describe("job publication routes", () => {
  it("sets advertisement details", async () => {
    ...
    const r = await app.inject({ method: "PATCH", url: `/v1/hrms/job-openings/${VAC}/advertisement`, headers: auth,
  ...
  it("records a corrigendum preserving the advert", async () => {
    const app = await buildApp();
    const r = await app.inject({ method: "POST", url: `/v1/hrms/job-openings/${VAC}/corrigendum`, headers: auth, payload: { changes: "qualification revised to B.E/B.Tech" } });
    expect(r.statusCode).toBe(200);
  ...
  it("extends the deadline (and reopens), rejecting an earlier date", async () => {
    const app = await buildApp();
    const bad = await app.inject({ method: "POST", url: `/v1/hrms/job-openings/${VAC}/extend`, headers: auth, payload: { newDeadline: "2026-08-01T00:00:00Z" } });
    expect(bad.statusCode).toBe(400);
    expect(bad.json().code).toBe("NOT_AN_EXTENSION");
    H.findMock.mockResolvedValue(vac({ status: "closed" }));
    const ok = await app.inject({ method: "POST", url: `/v1/hrms/job-openings/${VAC}/extend`, headers: auth, payload: { newDeadline: "2026-09-30T00:00:00Z", reason: "low response" } });
  ...
  it("cancels a vacancy and blocks double-cancel", async () => {
    const app = await buildApp();
    const r = await app.inject({ method: "POST", url: `/v1/hrms/job-openings/${VAC}/cancel`, headers: auth, payload: { reason: "post abolished" } });
    ...
    const again = await app.inject({ method: "POST", url: `/v1/hrms/job-openings/${VAC}/cancel`, headers: auth, payload: { reason: "x" } });
  ...
  it("lists corrigenda history", async () => {
    ...
    const app = await buildApp();
    const r = await app.inject({ method: "GET", url: `/v1/hrms/job-openings/${VAC}/corrigenda`, headers: auth });
  ...
  it("public career search returns an advert-safe projection", async () => {
    ...
    const app = await buildApp();
    const r = await app.inject({ method: "GET", url: `/v1/careers/search?tenantId=${TENANT}&keyword=scientist&location=HQ` });
    expect(r.statusCode).toBe(200);
