...
describe("qualification requirement + validate routes", () => {
  it("sets a requirement (200)", async () => {
    ...
    const r = await app.inject({ method: "PUT", url: `/v1/hrms/job-openings/${JOB}/qualification-requirement`, headers: auth, payload: { minTotalYears: 3, minRelevantYears: 1, minEducationLevel: "bachelor", requiredDisciplines: ["Computer Science"] } });
  ...
  it("rejects relevant-years greater than total-years (422)", async () => {
    const app = await buildApp();
    const r = await app.inject({ method: "PUT", url: `/v1/hrms/job-openings/${JOB}/qualification-requirement`, headers: auth, payload: { minTotalYears: 2, minRelevantYears: 3 } });
  ...
  it("refuses to validate when no requirement is set (409)", async () => {
    H.findByJob.mockResolvedValue(null);
    const app = await buildApp();
    const r = await app.inject({ method: "POST", url: `/v1/hrms/job-openings/${JOB}/validate-candidate`, headers: auth, payload: { experience: [], education: [] } });
  ...
  it("validates an eligible candidate against the stored requirement (200)", async () => {
    ...
    const app = await buildApp();
    const r = await app.inject({ method: "POST", url: `/v1/hrms/job-openings/${JOB}/validate-candidate`, headers: auth, payload: {
