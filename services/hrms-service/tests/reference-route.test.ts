...
describe("candidate reference/reservation routes", () => {
  it("sets reservation attributes with a certificate (200)", async () => {
    ...
    const r = await app.inject({ method: "PUT", url: `/v1/hrms/candidates/${CID}/reservation-attributes`, headers: auth, payload: { category: "OBC", reservationDocs: ["cert-1"] } });
  ...
  it("rejects a reserved-category claim without a certificate (422)", async () => {
    const app = await buildApp();
    const r = await app.inject({ method: "PUT", url: `/v1/hrms/candidates/${CID}/reservation-attributes`, headers: auth, payload: { category: "SC" } });
  ...
  it("rejects a disability claim missing type/percentage (422)", async () => {
    const app = await buildApp();
    const r = await app.inject({ method: "PUT", url: `/v1/hrms/candidates/${CID}/reservation-attributes`, headers: auth, payload: { disability: true, reservationDocs: ["d"] } });
    expect(r.statusCode).toBe(422);
    await app.close();
  });
  ...
  it("sets two references (200) and rejects a single reference (422)", async () => {
    const app = await buildApp();
    const ok = await app.inject({ method: "PUT", url: `/v1/hrms/candidates/${CID}/references`, headers: auth, payload: { references: twoRefs } });
    expect(ok.statusCode).toBe(200);
    expect(H.setReferences).toHaveBeenCalledOnce();
    const bad = await app.inject({ method: "PUT", url: `/v1/hrms/candidates/${CID}/references`, headers: auth, payload: { references: [twoRefs[0]] } });
  ...
  it("rejects a reference that is the candidate (422)", async () => {
    const app = await buildApp();
    const r = await app.inject({ method: "PUT", url: `/v1/hrms/candidates/${CID}/references`, headers: auth, payload: { references: [
  ...
  it("rejects an unknown reservation category (422)", async () => {
    const app = await buildApp();
    const r = await app.inject({ method: "PUT", url: `/v1/hrms/candidates/${CID}/reservation-attributes`, headers: auth, payload: { category: "XX", reservationDocs: ["d"] } });
    expect(r.statusCode).toBe(422);
    expect(r.json().code).toBe("INVALID_RESERVATION");
    await app.close();
  });
  ...
  it("normalises the persisted category to uppercase", async () => {
    const app = await buildApp();
    await app.inject({ method: "PUT", url: `/v1/hrms/candidates/${CID}/reservation-attributes`, headers: auth, payload: { category: "obc", reservationDocs: ["c"] } });
  ...
  it("records a prior-relationship declaration (200) and requires named relations (422)", async () => {
    const app = await buildApp();
    const ok = await app.inject({ method: "PUT", url: `/v1/hrms/candidates/${CID}/relationship-declaration`, headers: auth, payload: { hasPriorRelationship: true, relations: [{ personName: "Panelist X", nature: "former colleague" }] } });
    expect(ok.statusCode).toBe(200);
    const bad = await app.inject({ method: "PUT", url: `/v1/hrms/candidates/${CID}/relationship-declaration`, headers: auth, payload: { hasPriorRelationship: true } });
    expect(bad.statusCode).toBe(422);
  ...
  it("clears relations when hasPriorRelationship is false", async () => {
    const app = await buildApp();
    await app.inject({ method: "PUT", url: `/v1/hrms/candidates/${CID}/relationship-declaration`, headers: auth, payload: { hasPriorRelationship: false, relations: [{ personName: "X", nature: "friend" }] } });
  ...
  it("locks edits after submission (409)", async () => {
    ...
    const app = await buildApp();
    const r = await app.inject({ method: "PUT", url: `/v1/hrms/candidates/${CID}/references`, headers: auth, payload: { references: twoRefs } });
