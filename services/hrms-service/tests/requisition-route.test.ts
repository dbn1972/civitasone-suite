...
describe("recruitment requisition routes", () => {
  it("creates a draft requisition (201)", async () => {
    ...
    const r = await app.inject({ method: "POST", url: "/v1/hrms/requisitions", headers: auth(["hr_admin"]),
  ...
  it("submits a draft into the approval chain at stage 0", async () => {
    ...
    const app = await buildApp();
    const r = await app.inject({ method: "POST", url: `/v1/hrms/requisitions/${REQ}/submit`, headers: auth(["hr_admin"]) });
  ...
  it("advances the chain only for the stage's configured role; logs the approval", async () => {
    ...
    const app = await buildApp();
    const r = await app.inject({ method: "POST", url: `/v1/hrms/requisitions/${REQ}/approve`, headers: auth(["hiring_manager"], OTHER), payload: { comments: "ok" } });
    expect(r.statusCode).toBe(200);
  ...
  it("blocks the creator from approving their own requisition (409 SOD)", async () => {
    ...
    const r = await app.inject({ method: "POST", url: `/v1/hrms/requisitions/${REQ}/approve`, headers: auth(["hiring_manager"], USER), payload: {} });
  ...
  it("blocks the same person from clearing a second stage (409 SOD)", async () => {
    ...
    const app = await buildApp();
    const r = await app.inject({ method: "POST", url: `/v1/hrms/requisitions/${REQ}/approve`, headers: auth(["hr_admin"], OTHER), payload: {} });
    expect(r.statusCode).toBe(409);
    expect(r.json().code).toBe("SOD_VIOLATION");
    await app.close();
  });
  ...
  it("forbids a non-admin from supplying a custom approval chain (403)", async () => {
    const app = await buildApp();
    const r = await app.inject({ method: "POST", url: "/v1/hrms/requisitions", headers: auth(["hiring_manager"]),
  ...
  it("lets an HR admin set a valid diverse custom chain (201) but rejects a rubber-stamp all-same-role chain (400)", async () => {
    const app = await buildApp();
    const ok = await app.inject({ method: "POST", url: "/v1/hrms/requisitions", headers: auth(["hr_admin"]),
      payload: { title: "X", departmentId: DEPT, approvalChain: [{ stage: "HM", role: "hiring_manager" }, { stage: "HR", role: "hr_admin" }] } });
    expect(ok.statusCode).toBe(201);
    const bad = await app.inject({ method: "POST", url: "/v1/hrms/requisitions", headers: auth(["hr_admin"]),
      payload: { title: "X", departmentId: DEPT, approvalChain: [{ stage: "HR1", role: "hr_admin" }, { stage: "HR2", role: "hr_admin" }] } });
    expect(bad.statusCode).toBe(400);
    const badRole = await app.inject({ method: "POST", url: "/v1/hrms/requisitions", headers: auth(["hr_admin"]),
  ...
  it("rejects approval by a role that is not the current stage (403)", async () => {
    ...
    const app = await buildApp();
    const r = await app.inject({ method: "POST", url: `/v1/hrms/requisitions/${REQ}/approve`, headers: auth(["hr_officer"]), payload: {} });
    expect(r.statusCode).toBe(403);
  ...
  it("marks approved when the final stage approves", async () => {
    ...
    const app = await buildApp();
    const r = await app.inject({ method: "POST", url: `/v1/hrms/requisitions/${REQ}/approve`, headers: auth(["competent_authority"], OTHER), payload: {} });
    expect(r.statusCode).toBe(200);
  ...
  it("returns for correction with mandatory comments and records history", async () => {
    ...
    const app = await buildApp();
    const noComment = await app.inject({ method: "POST", url: `/v1/hrms/requisitions/${REQ}/return`, headers: auth(["hr_admin"]), payload: {} });
    expect(noComment.statusCode).toBe(400); // comments mandatory
    const r = await app.inject({ method: "POST", url: `/v1/hrms/requisitions/${REQ}/return`, headers: auth(["hr_admin"]), payload: { comments: "fix grade" } });
    expect(r.statusCode).toBe(200);
  ...
  it("blocks publication until fully approved (R-RA-0056)", async () => {
    ...
    const app = await buildApp();
    const r = await app.inject({ method: "POST", url: `/v1/hrms/requisitions/${REQ}/publish`, headers: auth(["hr_admin"]), payload: {} });
    expect(r.statusCode).toBe(409);
  ...
  it("clones a requisition into a fresh draft", async () => {
    ...
    const app = await buildApp();
    const r = await app.inject({ method: "POST", url: `/v1/hrms/requisitions/${REQ}/clone`, headers: auth(["hr_admin"]) });
    expect(r.statusCode).toBe(201);
    expect(r.json().status).toBe("draft");
  ...
  it("hides a confidential requisition from a non-privileged non-creator (404)", async () => {
    ...
    const app = await buildApp();
    const r = await app.inject({ method: "GET", url: `/v1/hrms/requisitions/${REQ}`, headers: auth(["hr_officer"]) });
  ...
  it("shows a confidential requisition to an HR admin", async () => {
    H.findReqMock.mockResolvedValue(reqRow({ confidential: true, createdBy: OTHER }));
    const app = await buildApp();
    const r = await app.inject({ method: "GET", url: `/v1/hrms/requisitions/${REQ}`, headers: auth(["hr_admin"]) });
    expect(r.statusCode).toBe(200);
    await app.close();
  });
});
