...
describe("selection list routes", () => {
  it("creates a draft list (201)", async () => {
    ...
    const r = await app.inject({ method: "POST", url: `/v1/hrms/job-openings/${JOB}/selection-lists`, headers: creator, payload: { title: "Merit 2026", vacancies: 2 } });
  ...
  it("sets ranked entries on a draft list (200)", async () => {
    ...
    const app = await buildApp();
    const r = await app.inject({ method: "PUT", url: `/v1/hrms/selection-lists/${LIST}/entries`, headers: creator, payload: { entries: goodEntries } });
  ...
  it("rejects entries with more selected than vacancies (422)", async () => {
    ...
    const app = await buildApp();
    const r = await app.inject({ method: "PUT", url: `/v1/hrms/selection-lists/${LIST}/entries`, headers: creator, payload: { entries: [
      { applicationId: A1, candidateName: "A", category: "selected", rank: 1 },
  ...
  it("blocks the creator from approving their own list (403 SoD)", async () => {
    ...
    const app = await buildApp();
    const r = await app.inject({ method: "POST", url: `/v1/hrms/selection-lists/${LIST}/approve`, headers: creator, payload: { validUntil: futureIso } });
  ...
  it("blocks whoever authored the ranking from approving (403 SoD), even if not the creator", async () => {
    ...
    H.listEntries.mockResolvedValue([{ applicationId: A1, category: "selected", rank: 1 }]);
    const app = await buildApp();
    const r = await app.inject({ method: "POST", url: `/v1/hrms/selection-lists/${LIST}/approve`, headers: approver, payload: { validUntil: futureIso } });
    expect(r.statusCode).toBe(403);
    expect(r.json().code).toBe("SOD_VIOLATION");
    await app.close();
  });
  ...
  it("approve is senior-only (hr_officer 403)", async () => {
    H.findList.mockResolvedValue(list());
    const app = await buildApp();
    const r = await app.inject({ method: "POST", url: `/v1/hrms/selection-lists/${LIST}/approve`, headers: officer, payload: { validUntil: futureIso } });
    expect(r.statusCode).toBe(403);
    await app.close();
  });
  ...
  it("publishes an approved list (200) and refuses to publish a draft (409)", async () => {
    H.findList.mockResolvedValue(list({ status: "approved" }));
    const app = await buildApp();
    const ok = await app.inject({ method: "POST", url: `/v1/hrms/selection-lists/${LIST}/publish`, headers: approver, payload: {} });
    expect(ok.statusCode).toBe(200);
    expect(ok.json().status).toBe("published");
    H.findList.mockResolvedValue(list({ status: "draft" }));
    const bad = await app.inject({ method: "POST", url: `/v1/hrms/selection-lists/${LIST}/publish`, headers: approver, payload: {} });
