...
describe("candidate professional-profile routes", () => {
  it("sets skills (200) and rejects bad proficiency (422)", async () => {
    ...
    const ok = await app.inject({ method: "PUT", url: `/v1/hrms/candidates/${CID}/skills`, headers: auth, payload: { skills: [{ skill: "SQL", proficiency: "expert", yearsExperience: 6 }] } });
    expect(ok.statusCode).toBe(200);
    expect(H.setSkills).toHaveBeenCalledOnce();
    const bad = await app.inject({ method: "PUT", url: `/v1/hrms/candidates/${CID}/skills`, headers: auth, payload: { skills: [{ skill: "SQL", proficiency: "guru" }] } });
  ...
  it("rejects a certification whose expiry precedes issue (422)", async () => {
    const app = await buildApp();
    const r = await app.inject({ method: "PUT", url: `/v1/hrms/candidates/${CID}/certifications`, headers: auth, payload: { certifications: [{ name: "PMP", issuer: "PMI", issueDate: "2022-01-01", expiryDate: "2021-01-01" }] } });
  ...
  it("rejects a language with no read/write/speak (422)", async () => {
    const app = await buildApp();
    const r = await app.inject({ method: "PUT", url: `/v1/hrms/candidates/${CID}/languages`, headers: auth, payload: { languages: [{ language: "Hindi" }] } });
    expect(r.statusCode).toBe(422);
  ...
  it("sets credentials (200)", async () => {
    const app = await buildApp();
    const r = await app.inject({ method: "PUT", url: `/v1/hrms/candidates/${CID}/credentials`, headers: auth, payload: { credentials: [{ kind: "patent", title: "A novel method", year: 2021 }] } });
  ...
  it("returns the professional profile (200)", async () => {
    ...
    const app = await buildApp();
    const r = await app.inject({ method: "GET", url: `/v1/hrms/candidates/${CID}/professional-profile`, headers: auth });
    expect(r.statusCode).toBe(200);
  ...
  it("locks edits after submission (409)", async () => {
    ...
    const app = await buildApp();
    const r = await app.inject({ method: "PUT", url: `/v1/hrms/candidates/${CID}/skills`, headers: auth, payload: { skills: [{ skill: "SQL", proficiency: "expert" }] } });
