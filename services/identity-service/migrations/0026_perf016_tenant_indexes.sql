...
-- leading-index backlog. This is the identity-service tranche-6 file: 2
...
-- 1590 tenant_id-bearing tables fleet-wide) -- identity: 2 missing,
...
--   users.api_key_rotations (status column: no)
--
...
-- information_schema, connected as identity_svc (not civitas_admin, matching
-- tranches 1-5's methodology).
--
-- Index naming: idx_<table>_tenant[_status]. Where the table's own bare
-- name already carries the identity_ prefix, it is reused verbatim (no double
-- prefixing); where it doesn't, identity_ is prepended -- same rule tranches
...
-- collisions across schemas in civitas_identity: none found for any table in
-- this file.
--
...
--
-- Rollback:
--   DROP INDEX CONCURRENTLY IF EXISTS _outbox.idx_identity_messages_legacy_tenant;
--   DROP INDEX CONCURRENTLY IF EXISTS users.idx_identity_api_key_rotations_tenant;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_identity_messages_legacy_tenant
  ON _outbox.messages_legacy (tenant_id);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_identity_api_key_rotations_tenant
  ON users.api_key_rotations (tenant_id);
