...
-- Mirrors works-service 0013_outbox_inbox_rls.sql / contract-service 0016.
-- Safe no-op if not owner.
--
-- Deploy note: inspection-worker MUST run startRelay(scannerDb, …) and
-- startOutboxPurge(scannerDb, …) (see src/worker.ts). The outbox relay/purge
-- are cross-tenant scans and inspection_scanner is BYPASSRLS (migration
-- 0022_inspection_scanner_role.sql) with grants on _outbox/_inbox.
--
-- Lesson learned from visitor-service (0013): inspection-service's
-- _inbox.processed (created in 0020_inbox_outbox_schema.sql) is
