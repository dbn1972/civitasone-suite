/**
 * Static regression: inspection-worker's outbox relay + scheduled purge must run
 * on the BYPASSRLS scannerDb, not the tenant-scoped inspection_svc `db`. Mirrors
 * court-service / visitor-service worker-relay-scanner.static.test.ts.
 */
import { describe, it, expect } from "vitest";
import { readFileSync } from "node:fs";
import { join } from "node:path";

describe("inspection worker outbox relay + purge use scannerDb", () => {
  const src = readFileSync(join(__dirname, "../src/worker.ts"), "utf8");

  it("imports scannerDb from shared/scanner-db.js", () => {
    expect(src).toMatch(/import\s*\{\s*scannerDb(?:\s*,\s*scannerSqlClient)?\s*\}\s*from\s*"\.\/shared\/scanner-db\.js"/);
  });
