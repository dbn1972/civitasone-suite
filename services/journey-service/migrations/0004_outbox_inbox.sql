...
--          journey-service calls enqueue() from @civitasone/outbox on every
...
-- Affected services: journey-service
SET lock_timeout = '5s';
...
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'journey_svc') THEN
    GRANT USAGE ON SCHEMA _outbox TO journey_svc;
    GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA _outbox TO journey_svc;
    ALTER DEFAULT PRIVILEGES IN SCHEMA _outbox
      GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO journey_svc;

    GRANT USAGE ON SCHEMA _inbox TO journey_svc;
    GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA _inbox TO journey_svc;
