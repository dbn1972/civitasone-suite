...
-- leading-index backlog. This is the knowledge-service tranche-6 file: 1
...
-- 1590 tenant_id-bearing tables fleet-wide) -- knowledge: 1 missing,
...
-- information_schema, connected as knowledge_svc (not civitas_admin, matching
-- tranches 1-5's methodology).
--
-- Index naming: idx_<table>_tenant[_status]. Where the table's own bare
-- name already carries the knowledge_ prefix, it is reused verbatim (no double
-- prefixing); where it doesn't, knowledge_ is prepended -- same rule tranches
...
-- collisions across schemas in civitas_knowledge: none found for any table in
-- this file.
--
...
--
-- Rollback:
--   DROP INDEX CONCURRENTLY IF EXISTS _outbox.idx_knowledge_messages_legacy_tenant;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_knowledge_messages_legacy_tenant
