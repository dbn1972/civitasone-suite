/**
 * Board-decision legal intake consumer — mock-based unit tests.
 *
 * Covers: intake creation from a `meeting.decision.legal` message; idempotent
 * replay (same messageId, and same decisionId under a fresh messageId); tenant
 * scoping; and malformed-payload drop.
 */
...
function makeMsg(payload: Record<string, unknown>, messageId = randomUUID()) {
  return {
    messageId, type: CONSUMED_EVENTS.boardDecisionLegal, tenantId: TENANT, actorId: ACTOR,
...
describe("meeting.decision.legal → board intake", () => {
  it("opens a pending_review intake item and emits audit", async () => {
    ...
    await q.publish(CONSUMED_EVENTS.boardDecisionLegal, makeMsg({
      decisionId, meetingId: randomUUID(), text: "Initiate recovery proceedings per board resolution",
      authority: "Legal Committee",
    }));
    await settle();
    ...
    expect(audit!.payload.service).toBe("legal");
    await q.stop();
  });
  ...
  it("drops a malformed payload (missing decisionId/text)", async () => {
    const q = new MemoryQueue(); registerBoardIntakeConsumers(q); await q.start();
    await q.publish(CONSUMED_EVENTS.boardDecisionLegal, makeMsg({ meetingId: randomUUID() }));
    await settle();
