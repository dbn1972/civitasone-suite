...
-- leading-index backlog. This is the location-service tranche-6 file: 2
...
-- 1590 tenant_id-bearing tables fleet-wide) -- location: 2 missing,
...
--   location.infrastructure_inspections (status column: no)
--
...
-- information_schema, connected as location_svc (not civitas_admin, matching
-- tranches 1-5's methodology).
--
-- Index naming: idx_<table>_tenant[_status]. Where the table's own bare
-- name already carries the location_ prefix, it is reused verbatim (no double
-- prefixing); where it doesn't, location_ is prepended -- same rule tranches
...
-- collisions across schemas in civitas_location: none found for any table in
-- this file.
--
...
--
-- Rollback:
--   DROP INDEX CONCURRENTLY IF EXISTS _outbox.idx_location_messages_legacy_tenant;
--   DROP INDEX CONCURRENTLY IF EXISTS location.idx_location_infrastructure_inspections_tenant;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_location_messages_legacy_tenant
  ON _outbox.messages_legacy (tenant_id);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_location_infrastructure_inspections_tenant
  ON location.infrastructure_inspections (tenant_id);
