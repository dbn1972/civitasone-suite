...
--          loyalty-service calls enqueue() from @civitasone/outbox on every
...
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'loyalty_svc') THEN
    GRANT USAGE ON SCHEMA _outbox TO loyalty_svc;
    GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA _outbox TO loyalty_svc;
    ALTER DEFAULT PRIVILEGES IN SCHEMA _outbox
      GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO loyalty_svc;

    GRANT USAGE ON SCHEMA _inbox TO loyalty_svc;
    GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA _inbox TO loyalty_svc;
