-- 0009_meeting_scanner_default_privileges_owner_fix.sql
-- Close the ALTER DEFAULT PRIVILEGES owner-role gap for meeting_scanner.
--
-- PURPOSE
-- 0007_meeting_scanner_role.sql ends with statements of the form:
--
--   ALTER DEFAULT PRIVILEGES FOR ROLE meeting_svc IN SCHEMA <schema>
--     GRANT SELECT ON TABLES TO meeting_scanner;
--
-- Default privileges are keyed to the role that CREATES the object, so that
-- rule only fires for tables created by meeting_svc. On every deployed
...
--
-- Nothing is broken today because 0007_meeting_scanner_role.sql also issues an explicit
...
--
-- The original FOR ROLE meeting_svc rule is deliberately NOT dropped: if
-- some environment does create tables as meeting_svc, that rule remains
-- correct for it. Both rules coexist harmlessly.
--
-- AFFECTED SERVICES
--   meeting-service (schemas: meeting)
--
...
--
-- ROLLBACK
--   ALTER DEFAULT PRIVILEGES FOR ROLE civitas_admin IN SCHEMA meeting
--     REVOKE SELECT ON TABLES FROM meeting_scanner;
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'meeting_scanner') THEN
    RAISE NOTICE 'role meeting_scanner absent - skipping default privilege fix';
    RETURN;
  END IF;

  EXECUTE 'ALTER DEFAULT PRIVILEGES FOR ROLE civitas_admin IN SCHEMA meeting GRANT SELECT ON TABLES TO meeting_scanner';
  EXECUTE 'GRANT SELECT ON ALL TABLES IN SCHEMA meeting TO meeting_scanner';
END
$$;
