/**
 * config-registry — data access.
 *
 * Two read paths, matching court/visitor:
 *   - Transactional (GUC-scoped) getters used by the consumer + by other modules'
 *     consumers on their own `tx`: getConfigForUpdate, insertConfig, listActiveKeys,
 *     getConfigValueOnTx. RLS is enforced by the caller's `app.tenant_id` GUC.
 *   - Route reads via scopedRead so RLS is enforced on the read path too.
 *   - loadNamespaceOverrides — a cross-tenant load for the maintenance workers,
 *     run through the BYPASSRLS scanner pool (shared/scanner-db.ts).
 */
 ...
 * Used by the effectiveAllowed pattern (e.g. resolving the tenant's permitted
 * committee types from the `meeting_committee_types` namespace).
 */
 ...
 * behavior is IDENTICAL when unconfigured.
 */
 ...
 * frequency) scan cross-tenant through the BYPASSRLS `scannerDb` pool, then resolve
 * each candidate's per-tenant policy in the loop. Reading overrides here — once per
 * cycle, no per-tenant round-trips, NO cache (the worker process must always see
 * the freshest DB value the API just wrote) — returns a
