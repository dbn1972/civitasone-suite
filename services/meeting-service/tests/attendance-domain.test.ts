...
describe("assertParticipantInvited (Req 6.2)", () => {
  it("accepts an invited participant of the meeting", () => {
    expect(() =>
      assertParticipantInvited({ id: PARTICIPANT, meetingId: MEETING, invitationStatus: "accepted" }, MEETING),
    ).not.toThrow();
    expect(() =>
      assertParticipantInvited({ id: PARTICIPANT, meetingId: MEETING, invitationStatus: "pending" }, MEETING),
    ).not.toThrow();
  });

  it("rejects a missing participant", () => {
    expect(() => assertParticipantInvited(null, MEETING)).toThrowError(/not an invited member/);
    expect(() => assertParticipantInvited(undefined, MEETING)).toThrowError(/not an invited member/);
  });

  it("rejects a participant from another meeting", () => {
    expect(() =>
      assertParticipantInvited({ id: PARTICIPANT, meetingId: OTHER_MEETING, invitationStatus: "accepted" }, MEETING),
    ).toThrowError(/does not belong to this meeting/);
  });

  it("rejects a declined/withdrawn invitation", () => {
    expect(() =>
      assertParticipantInvited({ id: PARTICIPANT, meetingId: MEETING, invitationStatus: "declined" }, MEETING),
