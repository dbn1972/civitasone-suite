...
export async function entityRoutes(app: FastifyInstance): Promise<void> {
  app.get("/v1/metadata/entities", async (req, reply) => {
    ...
    const entities = await db.select().from(entityDefinitions).where(eq(entityDefinitions.tenantId, ctx.tenantId));
  ...
  app.get("/v1/metadata/entities/:id", async (req, reply) => {
    ...
    const rows = await db.select().from(entityDefinitions).where(and(eq(entityDefinitions.id, id), eq(entityDefinitions.tenantId, ctx.tenantId))).limit(1);
