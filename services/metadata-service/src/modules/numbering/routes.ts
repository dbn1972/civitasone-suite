...
export async function numberingRoutes(app: FastifyInstance): Promise<void> {
  app.post("/v1/metadata/number-formats", async (req, reply) => {
    ...
    const row = await withTenant(ctx.tenantId, async (tx) => {
      const [created] = await tx.insert(numberFormats).values({
  ...
  app.patch("/v1/metadata/number-formats/:id", async (req, reply) => {
    ...
    const row = await withTenant(ctx.tenantId, async (tx) => {
      ...
      // Validate the resulting merged spec before persisting.
      const merged = {
      ...
      const [updated] = await tx.update(numberFormats).set(set)
        .where(and(eq(numberFormats.id, id), eq(numberFormats.tenantId, ctx.tenantId))).returning();
      return updated;
    });
  ...
  app.post("/v1/metadata/number-formats/:id/publish", async (req, reply) => {
    ...
    const row = await withTenant(ctx.tenantId, async (tx) => {
      ...
      // Re-validate the spec at publish time.
      rowToSpec(existing[0]);
      const [updated] = await tx.update(numberFormats)
        .set({ status: "active", publishedAt: new Date(), publishedBy: ctx.actorId, updatedAt: new Date(), updatedBy: ctx.actorId })
        .where(and(eq(numberFormats.id, id), eq(numberFormats.tenantId, ctx.tenantId))).returning();
      return updated;
    });
    return reply.send({ data: row });
