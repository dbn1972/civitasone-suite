 ...
 *   - The salt is read from PII_ENC_SALT when configured (recommended); a fixed,
 ...
 *     already high-entropy; configuring PII_ENC_SALT is the better posture.
 *
 * KEYRING CONFIG (rotation):
 *   - The active write key id defaults to "k1" (override with PII_KEY_ID).
 *   - Additional retired keys may be supplied via PII_ENC_KEYRING as a JSON
...
// Fixed, documented fallback salt (used only when PII_ENC_SALT is unset). 16
// bytes, ASCII. Configuring PII_ENC_SALT is preferred.
const DEFAULT_SALT = "civitas-hrms-pii";
