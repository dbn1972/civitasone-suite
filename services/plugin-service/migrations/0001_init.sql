-- plugin-service initial migration. Applied with the plugin_svc role on civitas_plugin.

CREATE TABLE IF NOT EXISTS plugin.items (
...
CREATE INDEX IF NOT EXISTS idx_items_tenant ON plugin.items(tenant_id);
