-- 0011_perf016_tenant_indexes.sql
-- PERF-016 (remainder of PERF-002): continues the fleet-wide tenant_id
-- leading-index backlog. This is the policy-service tranche-6 file: 1
...
-- 1590 tenant_id-bearing tables fleet-wide) -- policy: 1 missing,
...
-- information_schema, connected as policy_svc (not civitas_admin, matching
-- tranches 1-5's methodology).
--
-- Index naming: idx_<table>_tenant[_status]. Where the table's own bare
-- name already carries the policy_ prefix, it is reused verbatim (no double
-- prefixing); where it doesn't, policy_ is prepended -- same rule tranches
...
-- collisions across schemas in civitas_policy: none found for any table in
-- this file.
--
...
--
-- Rollback:
--   DROP INDEX CONCURRENTLY IF EXISTS _outbox.idx_policy_messages_legacy_tenant;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_policy_messages_legacy_tenant
