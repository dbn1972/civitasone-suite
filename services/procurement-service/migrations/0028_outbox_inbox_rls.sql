...
-- Mirrors court-service 0015_outbox_inbox_rls.sql / visitor-service
-- 0013_outbox_inbox_rls.sql / works-service 0011_outbox_rls_if_owner.sql.
-- Safe no-op if not owner.
--
-- Deploy note: procurement-worker MUST run startRelay(scannerDb, …) and
-- startOutboxPurge(scannerDb, …) (see src/worker.ts). The outbox relay/purge
-- are cross-tenant scans and procurement_scanner is BYPASSRLS (migration
-- 0027_procurement_scanner_role.sql) with grants on _outbox/_inbox.
--
-- Lesson learned from visitor-service (0013): the shared @civitasone/outbox
-- package's _inbox.processed table (packages/outbox/src/index.ts) is
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    RETURN;
  END IF;
  -- procurement-service's inbox (@civitasone/outbox shared package) is
  -- message_id-only — no tenant column to scope a tenant_isolation policy on.
  -- Skip safely rather than fabricate one (visitor-service 0013 lesson).
  IF NOT EXISTS (
