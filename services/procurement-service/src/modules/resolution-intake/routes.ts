/**
 * Resolution Indent Intake — routes (role-gated, RLS-scoped).
 *
 * GET   /v1/procurement/resolution-intake            — list intake items (default: pending_review)
 * PATCH /v1/procurement/resolution-intake/:id/review — accept/reject with optional note (maker-checker)
 *
 * Accept does NOT auto-create an indent; it marks the item reviewed so a competent
 * officer proceeds via the normal indent flow (see commands.ts hook comment).
 */
...
const ACCEPT_ROLES = ["procurement_admin", "super_admin"];

export async function resolutionIntakeRoutes(app: FastifyInstance): Promise<void> {
  app.get("/v1/procurement/resolution-intake", async (req, reply) => {
  ...
  app.patch("/v1/procurement/resolution-intake/:id/review", async (req, reply) => {
    const ctx = resolveContext(req);
    const { id } = intakeIdParam.parse(req.params);
    const { decision, note } = reviewBody.parse(req.body);

    // Maker-checker: only approver roles may ACCEPT; reject is allowed to all proc roles.
    requireRole(ctx, decision === "accepted" ? ACCEPT_ROLES : PROC_ROLES);
