...
export interface DerivedMatch {
  ...
  /** DOM-032: invoice date as supplied by the client, stored unparsed (see schema.ts). Same conditional-preserve semantics as invoiceRef below. */
...
export async function upsertDerivedMatch(tx: Writer, m: DerivedMatch): Promise<void> {
  await (tx as typeof db).execute(sql`
    INSERT INTO procurement.three_way_match
      (id, tenant_id, po_id, grn_id, invoice_id, po_amount_minor, grn_amount_minor, invoice_amount_minor, invoice_ref, invoice_date, match_status, variance_pct, auto_matched,
    ...
    VALUES (
      ...
      ${(m.invoiceAmountMinor ?? 0n).toString()}::bigint, ${m.invoiceRef ?? null}, ${m.invoiceDate ?? null}, ${m.matchStatus}, ${m.variancePct ?? null}, ${m.autoMatched ?? true},
    ...
    ON CONFLICT (tenant_id, po_id, grn_id) DO UPDATE SET
      ...
      invoice_date         = CASE WHEN EXCLUDED.invoice_id IS NOT NULL
                                  THEN EXCLUDED.invoice_date
                                  ELSE procurement.three_way_match.invoice_date END,
