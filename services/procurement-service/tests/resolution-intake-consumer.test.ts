/**
 * Resolution Indent Intake — consumer + RLS integration tests (DB-backed).
 *
 * Proves the cross-service choreography:
 *  - meeting.decision.procurement -> a PENDING REVIEW intake row (no auto-indent)
 ...
 *
 * Runs against the live civitas_procurement DB (DATABASE_URL from vitest.config.ts).
 * RLS-aware: seed + query paths run inside runWithTenant so SET LOCAL app.tenant_id
 * is applied and the indent.current_tenant_id() policy is enforced.
 */
...
const TOPIC = "meeting.decision.procurement";
...
function makeMsg(tenantId: string, decisionId: string, overrides: Record<string, unknown> = {}) {
  return {
    ...
    payload: {
      ...
      text: "Board resolved to procure 50 laptops for the district e-seva centres.",
      authority: "Purchase Committee",
  ...
  return runWithTenant(tenantId, () =>
    db.transaction(async (tx) => {
      const r: any = await tx.execute(
        sql`SELECT count(*)::int AS c FROM indent.procurement_resolution_indent_intake`,
      );
...
afterAll(async () => {
  for (const tid of [TENANT_A, TENANT_B]) {
    await runWithTenant(tid, () =>
      db.transaction(async (tx) => {
        await tx.execute(
          sql`DELETE FROM indent.procurement_resolution_indent_intake WHERE tenant_id = ${tid}::uuid`,
        );
      }),
    );
  }
...
describe("resolution-intake consumer (meeting.decision.procurement)", () => {
  const decisionId = randomUUID();

  it("creates a pending_review intake row from a board procurement decision", async () => {
    ...
    expect(row.authority).toBe("Purchase Committee");
  });

  it("does NOT auto-create any real indent (intake only)", async () => {
    const res = await runWithTenant(TENANT_A, () =>
      db.transaction(async (tx) => {
        const r: any = await tx.execute(
          sql`SELECT count(*)::int AS c FROM indent.procurement_indents WHERE tenant_id = ${TENANT_A}::uuid`,
        );
        const rows = Array.isArray(r) ? r : r.rows ?? [];
        return Number(rows[0]?.c ?? 0);
      }),
  ...
  it("replay of the SAME decisionId with a NEW messageId does not duplicate", async () => {
    ...
    const res = await runWithTenant(TENANT_A, () =>
      db.transaction(async (tx) => {
        const r: any = await tx.execute(
          sql`SELECT count(*)::int AS c FROM indent.procurement_resolution_indent_intake WHERE tenant_id = ${TENANT_A}::uuid AND decision_id = ${dId}::uuid`,
        );
        const rows = Array.isArray(r) ? r : r.rows ?? [];
        return Number(rows[0]?.c ?? 0);
      }),
