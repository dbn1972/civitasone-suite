...
async function wipe() {
  ...
  await db.delete(projectFundReleases).where(eq(projectFundReleases.tenantId, TENANT));
  await db.delete(projectSchemeComponents).where(eq(projectSchemeComponents.tenantId, TENANT));
  await db.delete(projectSchemes).where(eq(projectSchemes.tenantId, TENANT));
...
describe("Scheme consumer — fund release exceeds allocation (integration)", () => {
  ...
  it("scheme + components seed + over-allocation → allocation_exceeded in outbox, no DB write", async () => {
    ...
    const releases = await db.select().from(projectFundReleases).where(eq(projectFundReleases.id, REL_1));
...
describe("CQRS — fund release within allocation (integration)", () => {
  ...
  it("component B allocation 5000000 → release 3000000 → approved, DB written, event emitted", async () => {
    ...
    const releases = await db.select().from(projectFundReleases).where(eq(projectFundReleases.id, REL_2));
