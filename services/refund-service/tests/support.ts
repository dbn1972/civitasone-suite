/**
 * Shared test support for refund-service's DB-backed integration suite.
 * Mirrors services/vendor-service/tests/support.ts and services/animal-service/
 * tests/support.ts (the fleet reference for this pattern).
 *
 * Writes are queue-first (202 Accepted, F3/CQRS pattern): a route publishes a
 * command, and the domain side-effect (DB row, outbox event, audit row) only
 * applies once the module's real consumer processes it off the shared infra
 * queue. Tests register the real consumers (mirroring src/worker.ts) and
 * drain the in-memory queue before asserting persisted state.
 */
...
export function hdr(
  sub = ACTOR_A,
  tid = TENANT_A,
  roles: string[] = ["refund_admin", "refund_approver", "super_admin"],
