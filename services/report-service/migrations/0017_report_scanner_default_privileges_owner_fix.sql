-- 0017_report_scanner_default_privileges_owner_fix.sql
-- Close the ALTER DEFAULT PRIVILEGES owner-role gap for report_scanner.
--
-- PURPOSE
-- 0014_report_scanner_role.sql ends with statements of the form:
--
--   ALTER DEFAULT PRIVILEGES FOR ROLE report_svc IN SCHEMA <schema>
--     GRANT SELECT ON TABLES TO report_scanner;
--
-- Default privileges are keyed to the role that CREATES the object, so that
-- rule only fires for tables created by report_svc. On every deployed
...
--
-- Nothing is broken today because 0014_report_scanner_role.sql also issues an explicit
...
--
-- The original FOR ROLE report_svc rule is deliberately NOT dropped: if
-- some environment does create tables as report_svc, that rule remains
-- correct for it. Both rules coexist harmlessly.
--
-- AFFECTED SERVICES
--   report-service (schemas: reports)
--
...
--
-- ROLLBACK
--   ALTER DEFAULT PRIVILEGES FOR ROLE civitas_admin IN SCHEMA reports
--     REVOKE SELECT ON TABLES FROM report_scanner;
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'report_scanner') THEN
    RAISE NOTICE 'role report_scanner absent - skipping default privilege fix';
    RETURN;
  END IF;

  EXECUTE 'ALTER DEFAULT PRIVILEGES FOR ROLE civitas_admin IN SCHEMA reports GRANT SELECT ON TABLES TO report_scanner';
  EXECUTE 'GRANT SELECT ON ALL TABLES IN SCHEMA reports TO report_scanner';
END
$$;
