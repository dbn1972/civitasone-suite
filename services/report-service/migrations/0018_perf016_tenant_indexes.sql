...
-- leading-index backlog. This is the report-service tranche-6 file: 1
...
-- 1590 tenant_id-bearing tables fleet-wide) -- report: 1 missing,
...
-- information_schema, connected as report_svc (not civitas_admin, matching
-- tranches 1-5's methodology).
--
-- Index naming: idx_<table>_tenant[_status]. Where the table's own bare
-- name already carries the report_ prefix, it is reused verbatim (no double
-- prefixing); where it doesn't, report_ is prepended -- same rule tranches
...
-- collisions across schemas in civitas_report: none found for any table in
-- this file.
--
...
--
-- Rollback:
--   DROP INDEX CONCURRENTLY IF EXISTS _outbox.idx_report_messages_legacy_tenant;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_report_messages_legacy_tenant
