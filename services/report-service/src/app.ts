...
export async function buildApp(): Promise<FastifyInstance> {
  ...
  // stays unset and -- under the NOBYPASSRLS report_svc role (#146) with
  // fail-closed RLS -- route transactions error and reads return 0 rows.
  app.addHook("onRequest", async (req) => {
    const tid = (req as { ctx?: { tenantId?: string } }).ctx?.tenantId;
    if (tid) tenantStorage.enterWith({ tenantId: tid });
  });

  registerOpsRoutes(app, { service: "report-service", checks: { db: { ping: () => dbPing(sqlClient) }, cache, queue } });
