/**
 * Assessee module — route-level integration tests.
 *
 * Covers: POST /assessees 202, PATCH /assessees/:id 202, GET /assessees/:id 200/404,
 * GET /assessees paginated, 400/401/403 error paths.
 */
...
describe("POST /v1/revenue/assessees", () => {
  ...
  it("returns 401 without auth", async () => {
    const res = await app.inject({ method: "POST", url: "/v1/revenue/assessees", payload: VALID_BODY });
    expect(res.statusCode).toBe(401);
  });

  it("returns 403 with wrong role", async () => {
    const res = await app.inject({ method: "POST", url: "/v1/revenue/assessees", headers: BAD_ROLE, payload: VALID_BODY });
    expect(res.statusCode).toBe(403);
  });
});

// ── PATCH /v1/revenue/assessees/:id ───────────────────────────────────────────

describe("PATCH /v1/revenue/assessees/:id", () => {
  const VALID_BODY = { version: 1, patch: { name: "Jane Doe" } };

  it("returns 202 with valid body", async () => {
    const res = await app.inject({ method: "PATCH", url: `/v1/revenue/assessees/${ASSESSEE_ID}`, headers: AUTH, payload: VALID_BODY });
    expect(res.statusCode).toBe(202);
    expect(res.json().data).toHaveProperty("messageId");
  });
  ...
  it("returns 401 without auth", async () => {
    const res = await app.inject({ method: "PATCH", url: `/v1/revenue/assessees/${ASSESSEE_ID}`, payload: VALID_BODY });
    expect(res.statusCode).toBe(401);
  });

  it("returns 403 with wrong role", async () => {
    const res = await app.inject({ method: "PATCH", url: `/v1/revenue/assessees/${ASSESSEE_ID}`, headers: BAD_ROLE, payload: VALID_BODY });
    expect(res.statusCode).toBe(403);
  });
});
...
describe("GET /v1/revenue/assessees/:id", () => {
  it("returns 404 when repo returns null", async () => {
    const res = await app.inject({ method: "GET", url: `/v1/revenue/assessees/${ASSESSEE_ID}`, headers: AUTH });
    expect(res.statusCode).toBe(404);
    expect(res.json().error.code).toBe("NOT_FOUND");
  });

  it("returns 401 without auth", async () => {
    const res = await app.inject({ method: "GET", url: `/v1/revenue/assessees/${ASSESSEE_ID}` });
    expect(res.statusCode).toBe(401);
  });

  it("returns 403 with wrong role", async () => {
    const res = await app.inject({ method: "GET", url: `/v1/revenue/assessees/${ASSESSEE_ID}`, headers: BAD_ROLE });
    expect(res.statusCode).toBe(403);
  });
...
// ── GET /v1/revenue/assessees (paginated) ─────────────────────────────────────

describe("GET /v1/revenue/assessees", () => {
  it("returns 200 with paginated response", async () => {
    const res = await app.inject({ method: "GET", url: "/v1/revenue/assessees", headers: AUTH });
  ...
  it("returns 401 without auth", async () => {
    const res = await app.inject({ method: "GET", url: "/v1/revenue/assessees" });
    expect(res.statusCode).toBe(401);
  });

  it("returns 403 with wrong role", async () => {
    const res = await app.inject({ method: "GET", url: "/v1/revenue/assessees", headers: BAD_ROLE });
    expect(res.statusCode).toBe(403);
  });
});
