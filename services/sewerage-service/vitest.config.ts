  ...
  test: {
    env: {
      ...
      DATABASE_URL:
        ...
        "postgres://sewerage_svc:sewerage_dev_pw@localhost:5435/civitas_sewerage",
      QUEUE_DRIVER: "memory",
      CACHE_DRIVER: "memory",
    },
    ...
    // already had three real-DB integration files (hardening, rls-raw,
    // routes-health) before this pair, for five total.
    //
    // building-service hit this exact situation (its own two new
    // DATABASE_URL-flipping cross-service files alongside an existing
    // real-DB suite, see services/building-service/vitest.config.ts) and
    // found the default `threads` pool reuses a small set of worker
    // threads across test files, so Node module state (@civitasone/db's
    // internal client caches, keyed off DATABASE_URL at import time) can
    // leak across files that land in the same worker — a file that flips
    // DATABASE_URL mid-run can hang a LATER file's beforeAll ("Hook timed
    // out") if they share a thread. `pool: "forks"` gives every test file
    // its own OS process, removing the shared module state entirely.
    //
    ...
    // not-yet-relayed outbox row. Same fix fleet-wide for this class of
    // bug: trade-service (PR #1022) and parking-service (PR #1026) hit a
    // sibling variant of it via a blanket TRUNCATE in an existing test file
    // racing a new one, and used this exact setting.
    pool: "forks",
    poolOptions: {
      forks: {
        singleFork: false,
