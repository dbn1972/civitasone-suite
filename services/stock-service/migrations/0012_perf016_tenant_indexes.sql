...
-- leading-index backlog. This is the stock-service tranche-6 file: 3
...
-- 1590 tenant_id-bearing tables fleet-wide) -- stock: 3 missing,
...
--   entry.stock_entries (status column: yes)
--   entry.stock_entry_items (status column: no)
--
...
-- information_schema, connected as stock_svc (not civitas_admin, matching
-- tranches 1-5's methodology).
--
-- Index naming: idx_<table>_tenant[_status]. Where the table's own bare
-- name already carries the stock_ prefix, it is reused verbatim (no double
-- prefixing); where it doesn't, stock_ is prepended -- same rule tranches
...
-- collisions across schemas in civitas_stock: none found for any table in
-- this file.
--
...
--
-- Rollback:
--   DROP INDEX CONCURRENTLY IF EXISTS _outbox.idx_stock_messages_legacy_tenant;
--   DROP INDEX CONCURRENTLY IF EXISTS entry.idx_stock_entries_tenant;
--   DROP INDEX CONCURRENTLY IF EXISTS entry.idx_stock_entries_tenant_status;
--   DROP INDEX CONCURRENTLY IF EXISTS entry.idx_stock_entry_items_tenant;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_stock_messages_legacy_tenant
  ON _outbox.messages_legacy (tenant_id);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_stock_entries_tenant
  ON entry.stock_entries (tenant_id);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_stock_entries_tenant_status
  ON entry.stock_entries (tenant_id, status);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_stock_entry_items_tenant
  ON entry.stock_entry_items (tenant_id);
