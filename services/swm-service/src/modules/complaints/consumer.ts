...
const log = pino({ name: "swm.complaints.consumer" });
...
export function registerComplaintConsumers(rawQueue: Queue): void {
  ...
  queue.subscribe(COMMANDS.complaintCreate, async (msg) => {
    ...
    await db.transaction(async (tx) => {
      ...
      await writeAudit(tx, ctxOf(msg), { action: "complaint.create", resourceType: "swm_complaint", resourceId: p.id });
    });
  ...
  queue.subscribe(COMMANDS.complaintAssign, async (msg) => {
    ...
    await db.transaction(async (tx) => {
      ...
      await writeAudit(tx, ctxOf(msg), { action: "complaint.assign", resourceType: "swm_complaint", resourceId: p.id });
    });
  ...
  queue.subscribe(COMMANDS.complaintResolve, async (msg) => {
    ...
    await db.transaction(async (tx) => {
      ...
      await writeAudit(tx, ctxOf(msg), { action: "complaint.resolve", resourceType: "swm_complaint", resourceId: p.id });
    });
  ...
  queue.subscribe(COMMANDS.complaintClose, async (msg) => {
    ...
    await db.transaction(async (tx) => {
      ...
      await writeAudit(tx, ctxOf(msg), { action: "complaint.close", resourceType: "swm_complaint", resourceId: p.id });
    });
