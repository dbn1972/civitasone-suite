...
import { swmComplaints, type ComplaintRow, type ComplaintInsert } from "./schema.js";
...
export async function findById(id: string, tenantId: string): Promise<ComplaintRow | null> {
  const rows = await scopedRead((tx) =>
    tx.select().from(swmComplaints).where(and(eq(swmComplaints.id, id), eq(swmComplaints.tenantId, tenantId))).limit(1),
  );
  return rows[0] ?? null;
}

export async function listByTenant(tenantId: string, limit: number, offset: number, status?: string) {
  const conditions = [eq(swmComplaints.tenantId, tenantId)];
  if (status) conditions.push(eq(swmComplaints.status, status));
  const where = and(...conditions);
  const rows = await scopedRead((tx) =>
    tx.select().from(swmComplaints).where(where).orderBy(desc(swmComplaints.createdAt)).limit(limit).offset(offset),
  );
  const countResult = await scopedRead((tx) =>
    tx.select({ count: sql<number>`count(*)::int` }).from(swmComplaints).where(where),
  );
  return { rows, total: countResult[0]?.count ?? 0 };
}

export async function insert(tx: ScopedTx, row: ComplaintInsert): Promise<void> {
  await tx.insert(swmComplaints).values(row);
}
...
export async function update(tx: ScopedTx, id: string, tenantId: string, patch: Partial<ComplaintInsert>, currentVersion: number): Promise<boolean> {
  const result = await tx
    .update(swmComplaints)
    .set({ ...patch, updatedAt: new Date(), version: sql`${swmComplaints.version} + 1` })
    .where(and(eq(swmComplaints.id, id), eq(swmComplaints.tenantId, tenantId), eq(swmComplaints.version, currentVersion)))
    .returning({ id: swmComplaints.id });
