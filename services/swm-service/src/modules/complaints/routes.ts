...
const createBody = z.object({
  ...
  complaintType: z.enum(["missed_collection", "spillage", "burning", "illegal_dumping", "overflow"]),
...
export async function complaintRoutes(app: FastifyInstance): Promise<void> {
  app.post("/v1/swm/complaints", async (req, reply) => {
  ...
  app.get("/v1/swm/complaints", async (req, reply) => {
    const ctx = resolveContext(req);
    requireRole(ctx, ROLES);
  ...
  app.get("/v1/swm/complaints/:id", async (req, reply) => {
    const ctx = resolveContext(req);
    requireRole(ctx, ROLES);
  ...
  app.post("/v1/swm/complaints/:id/assign", async (req, reply) => {
    const ctx = resolveContext(req);
  ...
  app.post("/v1/swm/complaints/:id/resolve", async (req, reply) => {
    const ctx = resolveContext(req);
    requireRole(ctx, ROLES);
    const { id } = idParam.parse(req.params);
  ...
  app.post("/v1/swm/complaints/:id/close", async (req, reply) => {
    const ctx = resolveContext(req);
    requireRole(ctx, ADMIN_ROLES);
    const { id } = idParam.parse(req.params);
