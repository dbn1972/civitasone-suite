...
-- leading-index backlog. This is the telephony-service tranche-6 file: 2
...
-- 1590 tenant_id-bearing tables fleet-wide) -- telephony: 2 missing,
...
--   telephony.broadcast_recipients (status column: yes)
--
...
-- information_schema, connected as telephony_svc (not civitas_admin, matching
-- tranches 1-5's methodology).
--
-- Index naming: idx_<table>_tenant[_status]. Where the table's own bare
-- name already carries the telephony_ prefix, it is reused verbatim (no double
-- prefixing); where it doesn't, telephony_ is prepended -- same rule tranches
...
-- collisions across schemas in civitas_telephony: none found for any table in
-- this file.
--
...
--
-- Rollback:
--   DROP INDEX CONCURRENTLY IF EXISTS _outbox.idx_telephony_messages_legacy_tenant;
--   DROP INDEX CONCURRENTLY IF EXISTS telephony.idx_telephony_broadcast_recipients_tenant;
--   DROP INDEX CONCURRENTLY IF EXISTS telephony.idx_telephony_broadcast_recipients_tenant_status;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_telephony_messages_legacy_tenant
  ON _outbox.messages_legacy (tenant_id);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_telephony_broadcast_recipients_tenant
  ON telephony.broadcast_recipients (tenant_id);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_telephony_broadcast_recipients_tenant_status
  ON telephony.broadcast_recipients (tenant_id, status);
