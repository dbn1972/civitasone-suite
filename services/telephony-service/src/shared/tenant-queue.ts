 ...
 *
 * WHY: telephony domain tables have FORCED row-level security whose policy is
 ...
 * row-level security policy"). This is the same fix visitor-service applies in
 * its worker; we apply it at consumer-registration so both the worker AND the
 * DB-backed tests get tenant-scoped handlers.
 *
