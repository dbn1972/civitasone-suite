/**
 * telephony-service tests — CQRS wiring with MemoryQueue + MemoryCache.
 */
import { describe, it, expect, beforeEach } from "vitest";
import { MemoryQueue } from "@civitasone/queue";
import { Cache, MemoryCache } from "@civitasone/cache";
import { createCallBody } from "../src/modules/calls/validators.js";

describe("call validators", () => {
  it("accepts minimal create body", () => {
    const body = createCallBody.parse({ name: "Sample Call" });
    expect(body.name).toBe("Sample Call");
  });

  it("rejects empty name", () => {
    expect(() => createCallBody.parse({ name: "" })).toThrow();
  });
});
...
describe("write-via-queue + read-via-cache", () => {
  ...
  it("command primes cache before async DB write", async () => {
    ...
    const projected = { id, tenantId, name: "Test Call", status: "active" };
    ...
    expect(store.get(id)?.name).toBe("Test Call");
  });
