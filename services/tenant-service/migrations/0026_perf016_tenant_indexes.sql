...
-- leading-index backlog. This is the tenant-service tranche-6 file: 3
...
-- 1590 tenant_id-bearing tables fleet-wide) -- tenant: 3 missing,
...
--   tenant.code_values (status column: no)
--   tenant.tenants (status column: yes)
--
...
-- information_schema, connected as tenant_svc (not civitas_admin, matching
-- tranches 1-5's methodology).
--
-- Index naming: idx_<table>_tenant[_status]. Where the table's own bare
-- name already carries the tenant_ prefix, it is reused verbatim (no double
-- prefixing); where it doesn't, tenant_ is prepended -- same rule tranches
...
-- collisions across schemas in civitas_tenant: none found for any table in
-- this file.
--
...
--
-- Rollback:
--   DROP INDEX CONCURRENTLY IF EXISTS _outbox.idx_tenant_messages_legacy_tenant;
--   DROP INDEX CONCURRENTLY IF EXISTS tenant.idx_tenant_code_values_tenant;
--   DROP INDEX CONCURRENTLY IF EXISTS tenant.idx_tenant_tenants_tenant;
--   DROP INDEX CONCURRENTLY IF EXISTS tenant.idx_tenant_tenants_tenant_status;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_tenant_messages_legacy_tenant
  ON _outbox.messages_legacy (tenant_id);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_tenant_code_values_tenant
  ON tenant.code_values (tenant_id);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_tenant_tenants_tenant
  ON tenant.tenants (tenant_id);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_tenant_tenants_tenant_status
  ON tenant.tenants (tenant_id, status);
