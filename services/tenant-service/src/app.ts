...
export async function buildApp(): Promise<FastifyInstance> {
  ...
  // SEC (deep-verification, 2026-08-27): tenant-service had NO rate limiting at
  // all, unlike sibling services (crm/finance/hrms/payroll all register this).
  // Baseline matches hrms-service's convention; msme-onboard additionally gets
  // its own much stricter per-route override below, since it is now the one
  // route in this service reachable without authentication.
  await registerRateLimit(app, {
    max: 300,
    timeWindow: "1 minute",
    keyGenerator: (req: FastifyRequest) =>
      (req as any).ctx?.actorId ?? req.ip ?? "anonymous",
  ...
  const { orgHierarchyRoutes } =
    await import("./modules/org-hierarchy/routes.js");
  await app.register(orgHierarchyRoutes);
  const { dataMigrationRoutes } =
    await import("./modules/data-migration/routes.js");
  ...
  const { consentExchangeRoutes } =
    await import("./modules/consent-exchange/routes.js");
  ...
  const { tenantExtensionRoutes } =
    await import("./modules/tenant-extensions/routes.js");
