-- trade-service initial migration
-- Applied with trade_svc role on civitas_trade.
...
DROP POLICY IF EXISTS tenant_isolation ON trade.trade_applications;
...
DROP POLICY IF EXISTS tenant_isolation ON trade.trade_scrutiny_records;
...
DROP POLICY IF EXISTS tenant_isolation ON trade.trade_licences;
...
DROP POLICY IF EXISTS tenant_isolation ON trade.licence_actions;
...
DROP POLICY IF EXISTS tenant_isolation ON trade.trade_renewals;
...
-- ── Grants ─────────────────────────────────────────────────────────
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'trade_svc') THEN
    GRANT USAGE ON SCHEMA _outbox TO trade_svc;
    GRANT USAGE ON SCHEMA _inbox TO trade_svc;
    GRANT SELECT, INSERT, UPDATE ON _outbox.messages TO trade_svc;
    GRANT SELECT, INSERT ON _inbox.processed TO trade_svc;
    GRANT USAGE ON SCHEMA trade TO trade_svc;
    GRANT SELECT, INSERT, UPDATE ON ALL TABLES IN SCHEMA trade TO trade_svc;
  END IF;
END $$;
