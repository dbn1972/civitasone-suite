...
export async function approvalRoutes(app: FastifyInstance): Promise<void> {
  app.post("/v1/trade/approvals/scrutiny", async (req, reply) => {
  ...
  app.get("/v1/trade/approvals/scrutiny", async (req, reply) => {
    const ctx = resolveContext(req);
    requireRole(ctx, OFFICER_ROLES);
  ...
  app.post("/v1/trade/approvals/scrutiny/:id/complete", async (req, reply) => {
    const ctx = resolveContext(req);
    requireRole(ctx, OFFICER_ROLES);
    ...
    return reply.code(202).send(await commands.completeScrutiny(ctx, id, body.findings, body.deficiencyDetails));
  });

  app.post("/v1/trade/approvals/decide", async (req, reply) => {
    const ctx = resolveContext(req);
    requireRole(ctx, OFFICER_ROLES);
