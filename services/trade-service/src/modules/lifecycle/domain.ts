export const RENEWAL_TYPES = ["renewal", "amendment", "surrender", "duplicate"] as const;
...
export function calculateRenewalFeeMinor(renewalType: string): bigint {
  switch (renewalType) {
    case "renewal": return 75000n;
    case "amendment": return 50000n;
    case "duplicate": return 25000n;
    case "surrender": return 0n;
...
export function canRequestRenewal(licenceStatus: string, renewalType: string): boolean {
  if (renewalType === "surrender") return licenceStatus === "active";
  return licenceStatus === "active" || licenceStatus === "expired";
}
