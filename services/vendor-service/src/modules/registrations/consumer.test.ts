/**
 * Regression test: GET /v1/vendor/registrations/:id serves through
 * cache.getOrLoad (60s TTL); submitRegistration/withdrawRegistration were
 * already correctly guarded on repo.updateStatus's boolean return, but
 * never invalidated that key on write (CLAUDE.md §6).
 */
...
vi.mock("./domain.js", () => ({
  calculateFeeMinor: vi.fn(() => 25000),
  generateRegistrationNumber: vi.fn(() => "VR-1"),
}));
...
describe.each([
  [COMMANDS.submitRegistration, { id: REG_ID }],
  [COMMANDS.withdrawRegistration, { id: REG_ID }],
