...
// -- which doesn't match this service's tests, co-located under
// src/modules/** the same way every other properly-configured service in
// this monorepo does (see e.g. services/hrms-service/vitest.config.ts).
// That's very likely *why* this service had zero test coverage: any test
// file placed the normal way would have been silently skipped ("No test
// files found") rather than run.
// This file intentionally omits `include` so vitest falls back to its own
// built-in default (`**/*.{test,spec}.ts`), which picks up both any
// co-located module tests AND the top-level tests/ directory added in this
// pass (mirroring animal-service/swm-service/trade-service's DB-backed
...
export default defineConfig({
  test: {
    env: {
      ...
      // scripts/ci/bootstrap-postgres.sh's SERVICE_DBS[vendor-service]),
