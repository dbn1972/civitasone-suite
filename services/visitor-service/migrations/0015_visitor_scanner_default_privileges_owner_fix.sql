-- 0015_visitor_scanner_default_privileges_owner_fix.sql
-- Close the ALTER DEFAULT PRIVILEGES owner-role gap for visitor_scanner.
--
-- PURPOSE
-- 0009_scanner_role.sql ends with statements of the form:
--
--   ALTER DEFAULT PRIVILEGES FOR ROLE visitor_svc IN SCHEMA <schema>
--     GRANT SELECT ON TABLES TO visitor_scanner;
--
-- Default privileges are keyed to the role that CREATES the object, so that
-- rule only fires for tables created by visitor_svc. On every deployed
...
--
-- Nothing is broken today because 0009_scanner_role.sql also issues an explicit
...
--
-- The original FOR ROLE visitor_svc rule is deliberately NOT dropped: if
-- some environment does create tables as visitor_svc, that rule remains
-- correct for it. Both rules coexist harmlessly.
--
-- AFFECTED SERVICES
--   visitor-service (schemas: visitor)
--
...
--
-- ROLLBACK
--   ALTER DEFAULT PRIVILEGES FOR ROLE civitas_admin IN SCHEMA visitor
--     REVOKE SELECT ON TABLES FROM visitor_scanner;
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'visitor_scanner') THEN
    RAISE NOTICE 'role visitor_scanner absent - skipping default privilege fix';
    RETURN;
  END IF;

  EXECUTE 'ALTER DEFAULT PRIVILEGES FOR ROLE civitas_admin IN SCHEMA visitor GRANT SELECT ON TABLES TO visitor_scanner';
  EXECUTE 'GRANT SELECT ON ALL TABLES IN SCHEMA visitor TO visitor_scanner';
END
$$;
