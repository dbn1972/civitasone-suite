/**
 * visitor-service: config-registry consumer.
 *
 ...
 *   version-guarded update of `visitor.config_entries` -> enqueue the
 ...
 *   worker.ts subscribe wrapper enters runWithTenant(msg.tenantId, …)), so the
 *   FORCE-RLS policy on config_entries re-checks every mutation.
 *
 * `commands.ts` mints the entry's deterministic `id` BEFORE publishing (stable
 * per (tenant, namespace, key)); this consumer inserts with that exact id so a
 * redelivery is an idempotent no-op and re-setting the same key is an upsert.
 *
 * Mirrors court-service/src/modules/config-registry/consumer.ts.
 */
...
export function registerConfigRegistryConsumers(queue: Queue): void {
  // ─── setConfig (create or version-guarded update) ────────────────────────
  queue.subscribe<SetConfigPayload>(COMMANDS.setConfig, async (msg) => {
    const p = msg.payload;
  ...
  queue.subscribe<DeactivateConfigPayload>(COMMANDS.deactivateConfig, async (msg) => {
    const p = msg.payload;
    await db.transaction(async (tx) => {
      if (!(await markProcessed(tx, msg.messageId))) return;
      ...
      await enqueue(tx, { topic: AUDIT_TOPIC, eventType: AUDIT_TOPIC, tenantId: msg.tenantId, actorId: msg.actorId, correlationId: msg.correlationId, payload: { service: "visitor-service", action: "process", resourceType: "config_registry", resourceId: p.configId, outcome: "success" } });
