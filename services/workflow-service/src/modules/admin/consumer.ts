...
export function registerAdminConsumers(queue: Queue): void {
  ...
  subscribeWithDlq<{ id: string; tenantId: string; roleRef: string; userId: string; reportsTo?: string; active: boolean }>(queue, COMMANDS.upsertRoleMember, async (msg) => {
    await db.transaction(async (tx) => {
      ...
      const p = msg.payload;
      await tx.insert(roleMembers).values({ tenantId: p.tenantId, roleRef: p.roleRef, userId: p.userId, reportsTo: p.reportsTo ?? null, active: p.active }).onConflictDoUpdate({ target: [roleMembers.tenantId, roleMembers.roleRef, roleMembers.userId], set: { reportsTo: p.reportsTo ?? null, active: p.active } });
