...
function view(r: AuthorityLimitRow) {
  return {
    ...
    escalateToRef: r.escalateToRef ?? null, status: r.status, approvedBy: r.approvedBy ?? null,
  };
}
...
export async function authorityRoutes(app: FastifyInstance): Promise<void> {
  app.post("/v1/workflow/authority/limits", async (req, reply) => {
    ...
    const body = z.object({
      scopeType: scopeEnum, scopeRef: z.string().min(1).max(128),
      authorityType: typeEnum.default("financial"), currency: z.string().min(1).max(8).default("INR"),
      maxAmount: zMoneyMinorString, effectiveFrom: z.string().regex(dateRe),
    ...
    return sendAccepted(reply, acceptedResponseSchema, await commands.createAuthorityLimit(ctx, {
      scopeType: body.scopeType, scopeRef: body.scopeRef, authorityType: body.authorityType,
      currency: body.currency, maxAmount: body.maxAmount, effectiveFrom: body.effectiveFrom,
      effectiveTo: body.effectiveTo ?? null, escalateToScopeType: body.escalateToScopeType ?? null,
      escalateToRef: body.escalateToRef ?? null, reason: body.reason ?? null,
    }));
  });
  ...
  app.post("/v1/workflow/authority/limits/:id/approve", async (req, reply) => {
    ...
    if (existing.createdBy === ctx.actorId) throw new HttpError(409, "SELF_APPROVAL_FORBIDDEN", "the checker must differ from the maker");
    if (existing.status !== "draft") throw new HttpError(409, "NOT_DRAFT", `limit is ${existing.status}, not draft`);
    return sendAccepted(reply, acceptedResponseSchema, await commands.approveAuthorityLimit(ctx, id));
  });
  ...
  app.delete("/v1/workflow/authority/limits/:id", async (req, reply) => {
    ...
    return sendAccepted(reply, acceptedResponseSchema, await commands.revokeAuthorityLimit(ctx, id));
  });

  app.get("/v1/workflow/authority/limits", async (req, reply) => {
    const ctx = resolveContext(req); requireRole(ctx, ROLES);
    const q = z.object({ limit: z.coerce.number().int().min(1).max(200).default(100), offset: z.coerce.number().int().min(0).default(0) }).parse(req.query);
  ...
  app.post("/v1/workflow/authority/check", async (req, reply) => {
    const ctx = resolveContext(req); requireRole(ctx, ROLES);
    const body = z.object({
      authorityType: typeEnum.default("financial"), amount: zMoneyMinorString,
    ...
    const decision = evaluateAuthority(limits, { scopes: body.scopes as Array<{ scopeType: AuthorityScope; scopeRef: string }> }, body.authorityType as AuthorityType, BigInt(body.amount), onDate);
