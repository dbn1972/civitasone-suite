...
describe("call-activity (sub-workflow)", () => {
  it("spawns a child instance and resumes the parent when the child completes", async () => {
    ...
    await complete((await tasksFor(tenant, parentId))[0]!, randomUUID()); // p_start → enter p_call → spawn child

    // a non-human call task is held at the parent, and a child instance exists.
    const pTasks = await tasksFor(tenant, parentId);
    ...
    const childInst = await getInstance(tenant, childId);
    ...
    const childTask = (await tasksFor(tenant, childId)).find((t) => t.status === "pending")!;
    await complete(childTask, randomUUID());

    expect((await getInstance(tenant, childId))?.status).toBe("completed");
    expect((await getInstance(tenant, parentId))?.status).toBe("completed");
    expect(await historyActions(tenant, parentId)).toContain("call");
    expect(await historyActions(tenant, parentId)).toContain("call_return");
  });
  ...
  it("rejects an ancestor cycle A→A (self-recursive call): fails the parent, no child spawned", async () => {
    ...
    await complete((await tasksFor(tenant, id))[0]!, randomUUID()); // s_start → enter s_call (would re-invoke same def)

    // cycle guard: the call is rejected and the parent fails; NO child instance.
    const children = await sqlAsTenant(tenant, sql`SELECT id FROM workflow.instances WHERE parent_instance_id = ${id}`);
    expect((children as unknown as unknown[]).length).toBe(0);
    expect((await getInstance(tenant, id))?.status).toBe("completed"); // failed/closed
    expect(await historyActions(tenant, id)).toContain("call_error");
  });
...
describe("DLQ — consumer attempts + dead-letter", () => {
  it("bumps the attempt counter and re-throws below the threshold, then dead-letters at the threshold", async () => {
    ...
    await expect(asTenant(tenant, () => wrapped(env))).rejects.toThrow("boom");
    await expect(asTenant(tenant, () => wrapped(env))).rejects.toThrow("boom");
    // attempt 3: at threshold → swallowed + dead-lettered.
    await expect(asTenant(tenant, () => wrapped(env))).resolves.toBeUndefined();
    expect(calls).toBe(3);

    const dead = await asTenant(tenant, () => dlq.listDeadLetters(tenant, "dead", 10, 0));
  ...
  it("clears the attempt counter on success (so a transient failure that recovers is not dead-lettered)", async () => {
    ...
    await expect(asTenant(tenant, () => wrapped(env))).rejects.toThrow("transient"); // attempt 1
    await expect(asTenant(tenant, () => wrapped(env))).resolves.toBeUndefined();     // attempt 2 succeeds

    // counter cleared, nothing dead-lettered.
    const dead = await asTenant(tenant, () => dlq.listDeadLetters(tenant, "dead", 10, 0));
    expect(dead).toHaveLength(0);
    const attempts = await sqlAsTenant(tenant, sql`SELECT * FROM workflow.consumer_attempts WHERE message_id = ${messageId}`);
  ...
  it("dead-letter insert is idempotent on (topic, message_id)", async () => {
    ...
    await asTenant(tenant, () => dlq.deadLetter("dup.topic", messageId, tenant, env, "err1", 5));
    await asTenant(tenant, () => dlq.deadLetter("dup.topic", messageId, tenant, env, "err2", 6));
    const dead = await asTenant(tenant, () => dlq.listDeadLetters(tenant, "dead", 10, 0));
  ...
  it("markRequeued moves a dead letter to 'requeued' once (idempotent replay guard)", async () => {
    const tenant = newTenant();
    const messageId = randomUUID();
    await asTenant(tenant, () => dlq.deadLetter("replay.topic", messageId, tenant, { messageId }, "err", 5));
    const dead = (await asTenant(tenant, () => dlq.listDeadLetters(tenant, "dead", 10, 0)))[0]!;
    const actor = randomUUID();
    expect(await asTenant(tenant, () => dlq.markRequeued(dead.id, tenant, actor))).toBe(true);
    // a second requeue is rejected (status is no longer 'dead').
    expect(await asTenant(tenant, () => dlq.markRequeued(dead.id, tenant, actor))).toBe(false);
  });
});
...
describe("assignment strategies + role_members", () => {
  async function addMember(tenant: string, role: string, userId: string, reportsTo: string | null = null): Promise<void> {
    await asTenant(tenant, () => db.transaction(async (tx) => tx.insert(roleMembers).values({ tenantId: tenant, roleRef: role, userId, ...(reportsTo ? { reportsTo } : {}), active: true })));
  }
  ...
  it("round_robin cycles deterministically through active role-holders", async () => {
    ...
    const picks: string[] = [];
    for (let i = 0; i < 6; i++) {
      const a = await asTenant(tenant, () => db.transaction((tx) => resolveAssignee(tx, tenant, role, "round_robin", null)));
      picks.push(a!);
    }
  ...
  it("least_loaded picks the role-holder with the fewest open pending tasks", async () => {
    ...
    await sqlAsTenant(tenant, sql`INSERT INTO workflow.instances (id, tenant_id, name, status, created_by, updated_by) VALUES (${instId}, ${tenant}, 'i', 'active', ${randomUUID()}, ${randomUUID()})`);
    for (let i = 0; i < 2; i++) {
      await asTenant(tenant, () => db.transaction(async (tx) => tx.insert(tasks).values({
    ...
    const pick = await asTenant(tenant, () => db.transaction((tx) => resolveAssignee(tx, tenant, role, "least_loaded", null)));
  ...
  it("hierarchy picks a role-holder reporting to the given manager", async () => {
    ...
    const pick = await asTenant(tenant, () => db.transaction((tx) => resolveAssignee(tx, tenant, role, "hierarchy", manager)));
    expect(pick).toBe(report);
  });

  it("returns null when there is no strategy or no candidates", async () => {
    const tenant = newTenant();
    expect(await asTenant(tenant, () => db.transaction((tx) => resolveAssignee(tx, tenant, "role", "none", null)))).toBeNull();
    expect(await asTenant(tenant, () => db.transaction((tx) => resolveAssignee(tx, tenant, "role", "round_robin", null)))).toBeNull(); // no members
    expect(await asTenant(tenant, () => db.transaction((tx) => resolveAssignee(tx, tenant, null, "round_robin", null)))).toBeNull();
  });
});
...
describe("SLA escalation sweeper", () => {
  it("escalates an overdue pending task (stamps escalated_at, bumps escalation_count, writes history)", async () => {
    ...
    const after = await asTenant(tenant, () => db.transaction(async (tx) => tx.select().from(tasks).where(eq(tasks.id, taskId))));
    expect(after[0]!.escalationCount).toBe(1);
    expect(after[0]!.escalatedAt).not.toBeNull();
    expect(await historyActions(tenant, instId)).toContain("escalate");
  });
...
describe("pre-breach reminders", () => {
  it("emits a reminder once a threshold is crossed and bumps reminder_count (not escalation_count)", async () => {
    ...
    expect(await historyActions(tenant, instId)).toContain("reminder");
  });
});
...
describe("deemed-approval timer sweeper", () => {
  it("fires a due deemed-approval timer task and publishes a completeTask(approve) command", async () => {
    ...
    await complete((await tasksFor(tenant, id))[0]!, randomUUID()); // start → enter timer node (spawns timer task)

    const timerTask = (await tasksFor(tenant, id)).find((t) => t.node_key === "t_timer" && t.status === "pending")!;
    expect(timerTask).toBeTruthy();
    expect(timerTask.fire_at).toBeTruthy();
    // force fire_at into the past so the sweeper picks it up.
    await sqlAsTenant(tenant, sql`UPDATE workflow.tasks SET fire_at = now() - interval '1 minute' WHERE id = ${timerTask.id}`);
    ...
    const after = await asTenant(tenant, () => db.transaction(async (tx) => tx.select().from(tasks).where(eq(tasks.id, timerTask.id as string))));
    expect(after[0]!.fireAt).toBeNull();
    expect(await historyActions(tenant, id)).toContain("timer_fire");
  });
});
