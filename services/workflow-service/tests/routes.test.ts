/**
 * workflow-service HTTP route tests (inject).
...
function makeToken(roles: string[] = ["workflow_user"]) {
  return signToken({ sub: "user-001", tid: TENANT, roles, sid: "sess-001" }, SECRET);
}

afterAll(async () => { await sqlClient.end(); });

describe("GET /v1/workflow/instances — shape", () => {
  ...
  it("tenant isolation: other tenant returns 200 empty data", async () => {
    ...
    const token = signToken({ sub: "u2", tid: other, roles: ["workflow_user"], sid: "s2" }, SECRET);
    const res = await app.inject({
      method: "GET",
      url: "/v1/workflow/instances",
...
describe("GET /v1/workflow/tasks — shape", () => {
  it("returns 200 with paginated shape", async () => {
    const app = await buildApp();
    const res = await app.inject({
      method: "GET",
...
describe("unauthenticated requests", () => {
  it("GET /v1/workflow/instances without token → 401", async () => {
    const app = await buildApp();
    const res = await app.inject({ method: "GET", url: "/v1/workflow/instances" });
    await app.close();
    expect(res.statusCode).toBe(401);
  });

  it("GET /v1/workflow/tasks without token → 401", async () => {
    const app = await buildApp();
