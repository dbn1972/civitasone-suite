/**
 * Static regression: workflow-worker's outbox relay + scheduled purge must run
 * on the BYPASSRLS scannerDb, not the tenant-scoped workflow_svc `db`. Mirrors
 * works-service / court-service worker-relay-scanner.static.test.ts.
 */
import { describe, it, expect } from "vitest";
import { readFileSync } from "node:fs";
import { join } from "node:path";

describe("workflow worker outbox relay + purge use scannerDb", () => {
