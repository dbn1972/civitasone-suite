  ...
  q.subscribe(COMMANDS.boqAddItem, async (msg) => {
    await db.transaction(async (tx) => {
      ...
      await tx.insert(boqItems).values({
        ...
        srItemId: (p.srItemId as string) ?? undefined,
        itemDescription: p.itemDescription as string,
        itemCode: (p.itemCode as string) ?? undefined,
        unit: p.unit as string,
        rate,
        quantity: String(quantity),
        numberVal: p.numberVal ? String(p.numberVal) : undefined,
        lengthVal: p.lengthVal ? String(p.lengthVal) : undefined,
        breadthVal: p.breadthVal ? String(p.breadthVal) : undefined,
        depthVal: p.depthVal ? String(p.depthVal) : undefined,
        scopeId: (p.scopeId as string) ?? undefined,
