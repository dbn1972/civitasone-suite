  ...
  q.subscribe(COMMANDS.preTenderCreate, async (msg) => {
    await db.transaction(async (tx) => {
      ...
      await tx.insert(preTenders).values({
        ...
        tenderType: (p.tenderType as string) ?? undefined,
        tenderCategory: (p.tenderCategory as string) ?? undefined,
        bidValidity: (p.bidValidity as number) ?? undefined,
        fees: p.fees ? BigInt(p.fees as string | number) : undefined,
        status: "draft",
  ...
  q.subscribe(COMMANDS.awardCreate, async (msg) => {
    await db.transaction(async (tx) => {
      ...
      await tx.insert(awards).values({
        ...
        agreementNumber: (p.agreementNumber as string) ?? undefined,
        workOrderNumber: (p.workOrderNumber as string) ?? undefined,
        workPeriodDays: (p.workPeriodDays as number) ?? undefined,
        billMode: (p.billMode as string) ?? undefined,
