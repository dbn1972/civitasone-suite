/**
 * Field-level PII encryption at rest (mirrors procurement-service / hrms-service
 * implementation — see SEC-011).
 *
...
const KEY_LEN = 32;
const DEFAULT_SALT = "civitas-works-pii";
 ...
 * so the worker never runs fail-open on contractor PII (DPDP Act). Throws the
 * same "PII_ENC_KEY is required…" error as the first encrypt/decrypt would,
 * just surfaced at boot instead of on the first contractor write.
 */
